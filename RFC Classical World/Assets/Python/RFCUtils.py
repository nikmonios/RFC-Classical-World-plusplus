...
##									gc.getMap().plot(x, y).setCulture(iCiv, iPlotCulture/3, True)
	...
	def doPilgrimage(self, unit, iReligion):
		...
		iPlayer = unit.getOwner()
		pPlayer = gc.getPlayer(iPlayer)
		utils.makeUnit(con.iJewishMissionary + iReligion, iPlayer, (unit.getX(), unit.getY()), 1)
		unit.kill(0, -1)
	...
	def canCollapse(self, iPlayer):
		...
		if iPlayer == con.iMauryans and sd.getCivilization(con.iMauryans) == con.iSungas and gc.getMap().plot(con.tPataliputra[0],con.tPataliputra[1]).getOwner() == con.iMauryans and utils.getHumanID() in [con.iTocharians, con.iVietnam, con.iPandyans, con.iGoguryeo]: # keep Buddhist Holy City/Shrine open for Pilgrims/Teachers
			return False
	...
	def canDeclareWar(self, iCiv, iTarget):
		...
		if not gc.getPlayer(iCiv).canContact(iTarget): bWar = False
