...
##		# < Mercenaries End   >
	...
	def updateMiscButtons( self ):
		...
		if ( CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_HIDE_ALL or CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_MINIMAP_ONLY ):
			...
			# correct era and have the Currency tech.
			if(gc.getActivePlayer().getNumCities() > 0 and gc.getActivePlayer().getCurrentEra() >= g_iStartingEra):
				...
				if (teamPlayer.isHasTech(con.iBanking)):
		...
		if (not CyEngine().isGlobeviewUp() and pHeadSelectedCity):
			...
			if (CyInterface().getInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_SELECTION):
				...
				if ( pHeadSelectedUnit.getOwner() == gc.getGame().getActivePlayer() and g_pSelectedUnit != pHeadSelectedUnit ):
					...
					# Great Saint
					if pUnit.getUnitType() in [con.iGreatProphet, con.iGreatProphet2, con.iGreatProphet3]:
					...
					bWonder = false
					apCityList = PyPlayer(iUnitOwner).getCityList()
					...
					if pUnit.getUnitType() == con.iSpy and pPlot.getOwner() == pUnit.getOwner() and gc.getTeam(pUnitOwner.getTeam()).isHasTech(con.iCurrency):
						iNumUnits = pPlot.getNumUnits()
						iCost = 0
						iNumBarbarianUnits = 0
						for i in range (iNumUnits):
							unit = pPlot.getUnit(i)
							if unit.getOwner() == con.iBarbarian:
								iCost += objMercenaryUtils.getVirtualMercenaryCost(unit.getUnitType(), "temp")
								#iCost += (gc.getUnitInfo(unit.getUnitType()).getProductionCost() * 2 + gc.getUnitInfo(unit.getUnitType()).getProductionCost() * (unit.getLevel()+1) + ((unit.getLevel()+1) * int(math.fabs(objMercenaryUtils.getExperienceLevel()) - objMercenaryUtils.getNextExperienceLevel())))
								iNumBarbarianUnits += 1
						if iNumBarbarianUnits > 0 and pUnitOwner.getGold() >= iCost:
							screen.appendMultiListButton( "BottomButtonContainer", gc.getTechInfo(con.iBanking).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 664, 664, False )
							screen.show( "BottomButtonContainer" )
							iCount = iCount + 1
							
					# silkworm stealing srpt
					pUnit = g_pSelectedUnit
					iUnitType = pUnit.getUnitType()
					pUnitOwner = gc.getPlayer(pUnit.getOwner())
					pPlot = CyMap().plot(pUnit.getX(), pUnit.getY())
					if sd.getSilkWorms(pUnit.getOwner()) == 0:
					#if pUnitOwner.getNumAvailableBonuses(con.iSilk) < 1:
						if pUnit.getUnitType() == con.iSpy or pUnit.getUnitType() == con.iGreatSpy:
							if not pUnit.isHasPromotion(con.iSilkworm):
								if gc.getTeam(pUnitOwner.getTeam()).isHasTech(con.iCropRotation):
									if pPlot.getBonusType(-1) == con.iSilk and pPlot.getImprovementType() == con.iPlantation and pPlot.getOwner() != pUnit.getOwner():
										screen.appendMultiListButton( "BottomButtonContainer", gc.getTechInfo(con.iHorticulture).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 680, 680, False )
										screen.show( "BottomButtonContainer" )
										iCount = iCount + 1
									
					pUnit = g_pSelectedUnit
					pPlot = CyMap().plot(pUnit.getX(), pUnit.getY())
					if pUnit.isHasPromotion(con.iSilkworm) and pUnit.getOwner() == pPlot.getOwner() and pPlot.getBonusType(-1) == -1 and pPlot.getFeatureType() == -1 and pPlot.getTerrainType() in [con.iGrassland, con.iPlains]:
						screen.appendMultiListButton( "BottomButtonContainer", gc.getTechInfo(con.iHorticulture).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 681, 681, False )
						screen.show( "BottomButtonContainer" )
						iCount = iCount + 1
						
					# secret diplomacy srpt
					pUnit = g_pSelectedUnit
					iUnitType = pUnit.getUnitType()
					iUnitOwner = pUnit.getOwner()
					...
					if pUnit.getUnitType() == con.iSpy and pPlot.isCity():
						if pPlot.getPlotCity().getNumRealBuilding(con.iPalace) > 0:
							if gc.getTeam(pUnitOwner.getTeam()).isAtWar(iPlotOwner) and utils.getYear() > con.tBirth[iPlotOwner] + 30:
								screen.appendMultiListButton( "BottomButtonContainer", gc.getTechInfo(con.iCurrency).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 682, 682, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1
	...
	def updateGameDataStrings( self ):
		...
		if ( CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY  and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_ADVANCED_START):
			...
			if (gc.getPlayer(ePlayer).isAlive()):
				...
				# edead: start stability icon # srpt stability conversion
				#iCount += 1
				#if not CyInterface().isCityScreenUp():
				...
				#if iPiety >= 0 and not CyInterface().isCityScreenUp():
	...
	def updateCityScreen( self ):
		...
		screen.addPanel( "BonusPane0", u"", u"", True, False, xResolution - 244, 94, 57, yResolution - 600, PanelStyles.PANEL_STYLE_CITY_COLUMNL ) # srpt made these values larger to accomodate more specialist types in the pane below. original values were 520 and 536.
		screen.hide( "BonusPane0" )
		screen.addScrollPanel( "BonusBack0", u"", xResolution - 242, 94, 157, yResolution - 616, PanelStyles.PANEL_STYLE_EXTERNAL )
		screen.hide( "BonusBack0" )

		screen.addPanel( "BonusPane1", u"", u"", True, False, xResolution - 187, 94, 68, yResolution - 600, PanelStyles.PANEL_STYLE_CITY_COLUMNC )
		screen.hide( "BonusPane1" )
		screen.addScrollPanel( "BonusBack1", u"", xResolution - 191, 94, 184, yResolution - 616, PanelStyles.PANEL_STYLE_EXTERNAL )
		screen.hide( "BonusBack1" )

		screen.addPanel( "BonusPane2", u"", u"", True, False, xResolution - 119, 94, 107, yResolution - 600, PanelStyles.PANEL_STYLE_CITY_COLUMNR )
		screen.hide( "BonusPane2" )
		screen.addScrollPanel( "BonusBack2", u"", xResolution - 125, 94, 205, yResolution - 616, PanelStyles.PANEL_STYLE_EXTERNAL )
							...
							if (not CyInterface().isScoresMinimized() or gc.getGame().getActivePlayer() == ePlayer):
								if (gc.getPlayer(ePlayer).isAlive() and not gc.getPlayer(ePlayer).isMinorCiv()):
									...
									if (gc.getPlayer(ePlayer).getTeam() == eTeam):
										...
										if (gc.getPlayer(ePlayer).isAlive()):
											if (gc.getPlayer(ePlayer).isHuman()): # srpt
										...
										#Rhye - start stability ## srpt stability conversion
										if (gc.getPlayer(ePlayer).isAlive()):
	...
	def handleInput (self, inputClass):
		...
		# edead: end
		# srpt Pilgrim button
		if (inputClass.getNotifyCode() == 11 and inputClass.getData1() >= 668 and inputClass.getData1() <= 678 and inputClass.getData2() >= 668 and inputClass.getData2() <= 678):
			iReligion = inputClass.getData1() - 668
			utils.doPilgrimage(g_pSelectedUnit, iReligion)
		# srpt barbarian bribe
		if (inputClass.getNotifyCode() == 11 and inputClass.getData1() == 665 and inputClass.getData2() == 665):
			rfccwaiw.doBarbarianBribe(g_pSelectedUnit, g_pSelectedUnit.getX(), g_pSelectedUnit.getY())
			self.updateSelectionButtons()
		# srpt barbarian mercenary hire
		if (inputClass.getNotifyCode() == 11 and inputClass.getData1() == 664 and inputClass.getData2() == 664):
			rfccwaiw.doBarbarianMercenaryHire(g_pSelectedUnit, g_pSelectedUnit.getX(), g_pSelectedUnit.getY(), g_pSelectedUnit.getOwner())
			self.updateSelectionButtons()
		# srpt silkworm button 1
		if (inputClass.getNotifyCode() == 11 and inputClass.getData1() == 680 and inputClass.getData2() == 680):
			utils.doSilkworm(g_pSelectedUnit)
			self.updateSelectionButtons()
		# srpt silkworm button 2
		if (inputClass.getNotifyCode() == 11 and inputClass.getData1() == 681 and inputClass.getData2() == 681):
			utils.doSilkwormCultivation(g_pSelectedUnit, g_pSelectedUnit.getX(), g_pSelectedUnit.getY())
			self.updateSelectionButtons()
		# srpt secret diplomacy
		if (inputClass.getNotifyCode() == 11 and inputClass.getData1() == 682 and inputClass.getData2() == 682):
			rfccwaiw.doSecretDiplomacy(g_pSelectedUnit, g_pSelectedUnit.getX(), g_pSelectedUnit.getY())
			self.updateSelectionButtons()
		return 0
