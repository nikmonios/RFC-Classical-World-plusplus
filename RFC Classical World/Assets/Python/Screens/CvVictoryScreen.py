...
from Victory import Victory # edead
...
class CvVictoryScreen:
	...
	def __init__(self, screenId):
		...
		self.TABLE_WIDTH_0 = 590 #380 #Rhye 350 #565 is the minimum for the longest string of UHV
		self.TABLE_WIDTH_1 = 0 #80
		self.TABLE_WIDTH_2 = 125 #150 #Rhye 180
		self.TABLE_WIDTH_3 = 120 #160 #Rhye 100
		self.TABLE_WIDTH_4 = 110 #150 #Rhye 180
		self.TABLE_WIDTH_5 = 95
	...
	def showVictoryConditionScreen(self):
		...
		for i in range(gc.getMAX_CIV_TEAMS()):
			if gc.getTeam(i).isMinorCiv(): # edead
				nCities += gc.getTeam(i).getNumCities() # edead
			#if (gc.getTeam(i).isAlive() and not gc.getTeam(i).isMinorCiv() and not gc.getTeam(i).isBarbarian()):
			if (gc.getTeam(i).getNumCities() > 0 and not gc.getTeam(i).isMinorCiv() and not gc.getTeam(i).isBarbarian()): # edead
				nRivals += 1
		...
		#iOurReligion = gc.getPlayer(self.iActivePlayer).getStateReligion() # edead
		#ourReligionPercent = gc.getGame().calculateReligionPercent(iOurReligion) # edead
		...
		victory = gc.getVictoryInfo(8)
		if gc.getGame().isVictoryValid(8):
			...
			for i in range(3):
				...
				i3KingdomsMarker = sd.get3KingdomsMarker()
				if iHuman == con.iQin and i3KingdomsMarker == 1:
					screen.setTableText(szTable, 0, iRow, localText.getText(con.tGoals[gc.getGame().getGameSpeedType()][con.iJin][i], ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY) # edead - added game speed type
				elif iHuman == con.iHan and i3KingdomsMarker == 2:
					screen.setTableText(szTable, 0, iRow, localText.getText(con.tGoals[gc.getGame().getGameSpeedType()][con.iJin][i], ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY) # edead - added game speed type
				
				elif iHuman == con.iRome:
					if not gc.getTeam(gc.getPlayer(con.iRome).getTeam()).isHasTech(con.iRomanEmpire):
			...
			screen.appendTableRow(szTable) # edead
			screen.appendTableRow(szTable)
			#srpt - end
		
		for iLoopVC in range(gc.getNumVictoryInfos()-1): # srpt
			...
			if gc.getGame().isVictoryValid(iLoopVC):
				...
				if (victory.isConquest()): #Rhye
					...
					screen.setTableText(szTable, 4, iRow, u"Rebel cities:", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY) # edead
					screen.setTableText(szTable, 5, iRow, unicode(nCities), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY) # edead
				...
				if (victory.getReligionPercent() > 0):			
					iRow = screen.appendTableRow(szTable)
					# edead: start
					religionPercent = victory.getReligionPercent()
					if iOurReligion == gc.getInfoTypeForString("RELIGION_ISLAM"):
						religionPercent += 10
					elif iOurReligion == gc.getInfoTypeForString("RELIGION_HELLENISM"):
						religionPercent += 5
					elif iOurReligion == gc.getInfoTypeForString("RELIGION_CONFUCIANISM"):
						religionPercent -= 5
					screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_PERCENT_RELIGION", (religionPercent, )), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
