...
class TerrainGenerator:
	...
	def __init__(self, iDesertPercent=25, iPlainsPercent=20, iWetlandPercent=15,
		...
		self.iWetlandPercent = iWetlandPercent

		self.iDesertTopPercent = 100
		self.iDesertBottomPercent = max(0,int(100-iDesertPercent))
		self.iWetlandTopPercent = 100
		self.iWetlandBottomPercent = max(0,int(100-iDesertPercent-iWetlandPercent))
		self.iPlainsTopPercent = 100
		self.iPlainsBottomPercent = max(0,int(100-iDesertPercent-iWetlandPercent-iPlainsPercent))
	...
	def initFractals(self):
		...
		self.wetland.fracInit(self.iWidth, self.iHeight, self.grain_amount, self.mapRand, self.iFlags, self.fracXExp, self.fracYExp)
		self.iWetlandTop = self.wetland.getHeightFromPercent(self.iWetlandTopPercent)
		self.iWetlandBottom = self.wetland.getHeightFromPercent(self.iWetlandBottomPercent)
		...
		self.terrainWetland = self.gc.getInfoTypeForString("TERRAIN_WETLAND")
			...
			for y in range(self.iHeight):
				...
				for iDirection in range(CardinalDirectionTypes.NUM_CARDINALDIRECTION_TYPES):
					...
					if not pPlot.isNone():
						...
						if ((terrainData[iIndex] == self.terrainDesert) and (terrainData[iOtherIndex] == self.terrainWetland)) or ((terrainData[iIndex] == self.terrainWetland) and (terrainData[iOtherIndex] == self.terrainDesert)):
	...
	def generateTerrainAtPlot(self,iX,iY):
		...
		if lat >= self.fSnowLatitude:
			...
			wetlandVal = self.wetland.getHeight(iX, iY)
			...
			elif ((wetlandVal >= self.iWetlandBottom) and (wetlandVal <= self.iWetlandTop) and plot.isFlatlands() and (lat >= self.fDesertBottomLatitude) and (lat < self.fDesertTopLatitude)):
				terrainVal = self.terrainWetland
...
class FeatureGenerator:
	def __init__(self, iJunglePercent=80, iForestPercent=60, #iMarshPercent=0,
	             jungle_grain=5, forest_grain=6, #marsh_grain=6,
		...
		#marsh_grain += self.gc.getWorldInfo(self.map.getWorldSize()).getFeatureGrainChange()

		self.jungle_grain = jungle_grain
		self.forest_grain = forest_grain
		#self.marsh_grain = marsh_grain
	...
	def __initFractals(self):
		...
		#self.marshes.fracInit(self.iGridW, self.iGridH, self.marsh_grain, self.mapRand, self.iFlags, self.fracXExp, self.fracYExp)
	...
	def __initFeatureTypes(self):
		...
		self.featureMarsh = self.gc.getInfoTypeForString("FEATURE_MARSH")
	...
	def addFeaturesAtPlot(self, iX, iY):
		...
		#if (pPlot.getFeatureType() == FeatureTypes.NO_FEATURE):
		self.addMarshesAtPlot(pPlot, iX, iY, lat)
	...
	def addJunglesAtPlot(self, pPlot, iX, iY, lat):
		if pPlot.canHaveFeature(self.featureJungle):
			...
			if self.iJungleTop >= iJungleHeight >= self.iJungleBottom + (self.iJungleTop - self.iJungleBottom)*self.gc.getClimateInfo(self.map.getClimate()).getJungleLatitude()*lat:
				pPlot.setFeatureType(self.featureForest, -1) #Jungle->Forest
	...
	def addMarshesAtPlot(self, pPlot, iX, iY, lat):
		if pPlot.canHaveFeature(self.featureMarsh):
			pPlot.setFeatureType(self.featureMarsh, -1)
...
class BonusBalancer:
	def __init__(self):
		...
		self.resourcesToBalance = ('BONUS_HORSE', 'BONUS_IRON', 'BONUS_TIMBER', 'BONUS_SULFUR')
