...
EventTriggeredData* CyPlayer::initTriggeredData(int /*EventTriggerTypes*/ eEventTrigger, bool bFire, int iCityId, int iPlotX, int iPlotY, int /*PlayerTypes*/ eOtherPlayer, int iOtherPlayerCityId, int /*ReligionTypes*/ eReligion, int /*CorporationTypes*/ eCorporation, int iUnitId, int /*BuildingTypes*/ eBuilding, std::wstring /*CvWString*/ szCustomText)
{
	return m_pPlayer ? m_pPlayer->initTriggeredData((EventTriggerTypes)eEventTrigger, bFire, iCityId, iPlotX, iPlotY, (PlayerTypes)eOtherPlayer, iOtherPlayerCityId, (ReligionTypes)eReligion, (CorporationTypes)eCorporation, iUnitId, (BuildingTypes)eBuilding, szCustomText) : NULL;
}
...
// edead - start
int CyPlayer::getDiplomacyModifier() const
{
	return m_pPlayer ? m_pPlayer->getDiplomacyModifier() : -1;
}

void  CyPlayer::changeDiplomacyModifier(int iChange)
{
	if (m_pPlayer)
		m_pPlayer->changeDiplomacyModifier(iChange);
}

int CyPlayer::getLuck() const
{
	return m_pPlayer ? m_pPlayer->getLuck() : -1;
}

void  CyPlayer::setLuck(int iNewValue)
{
	if (m_pPlayer)
		m_pPlayer->setLuck(iNewValue);
}

int CyPlayer::AI_getUnitAIModifier(int /*UnitAITypes*/ eUnitAI) const
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		return pPlayer->AI_getUnitAIModifier((UnitAITypes)eUnitAI);
	return 0;
}

void  CyPlayer::AI_setUnitAIModifier(int /*UnitAITypes*/ eUnitAI, int iNewValue)
{
	CvPlayerAI* pPlayer = dynamic_cast<CvPlayerAI*>(m_pPlayer);
	if (pPlayer)
		pPlayer->AI_setUnitAIModifier((UnitAITypes)eUnitAI, iNewValue);
}
...
int CyPlayer::getBorders(int i)
{
	return m_pPlayer ? m_pPlayer->getBorders(i) : -1;
}
