...
void CvMap::calculateAreas()
{
	...
	// edead: start continents, from Rhye's
	
	// Original continent
	// CvArea* anatolia = addArea(); // Anatolia and Thrace
	// int anatoliaID = anatolia->getID();
	// anatolia->init(anatoliaID, false);
	
	// CvArea* levant = addArea(); // Egypt and Levant
	// int levantID = levant->getID();
	// levant->init(levantID, false);
	
	// CvArea* caucasus = addArea();
	// int caucasusID = caucasus->getID();
	// caucasus->init(caucasusID, false);
	
	// CvArea* iran = addArea(); // Greater Iran incl. Transoxiana and Hindu Kush
	// int iranID = iran->getID();
	// iran->init(iranID, false);
	
	// CvArea* hindustan = addArea();
	// int hindustanID = hindustan->getID();
	// hindustan->init(hindustanID, false);
	
	CvArea* oman = addArea();
	int omanID = oman->getID();
	oman->init(omanID, false);
	
	CvArea* bahrain = addArea();
	int bahrainID = bahrain->getID();
	bahrain->init(bahrainID, false);
	
	CvArea* mahra = addArea();
	int mahraID = mahra->getID();
	mahra->init(mahraID, false);
	
	// CvArea* arabia = addArea(); // Arabia and Mesopotamia
	// int arabiaID = arabia->getID();
	// arabia->init(arabiaID, false);
	
	// changed to one big Persia+India continent
	// for (iX = 0; iX < EARTH_X; iX++)
	// {
		// gDLL->callUpdater();
		// for (iY = 0; iY < EARTH_Y; iY++)
		// {
			// for (iI = 0; iI < NUM_IRAN_REGIONS; iI++)
			// {
				// if (regionMap[EARTH_Y -1 -iY][iX] == iranRegions[iI])
				// {
					// if (!plotSorenINLINE(iX, iY)->isWater())
						// plotSorenINLINE(iX, iY)->setArea(iranID);
				// }
			// }
			// for (iI = 0; iI < NUM_HINDUSTAN_REGIONS; iI++)
			// {
				// if (regionMap[EARTH_Y -1 -iY][iX] == hindustanRegions[iI])
				// {
					// if (!plotSorenINLINE(iX, iY)->isWater())
						// plotSorenINLINE(iX, iY)->setArea(iranID);
				// }
			// }
			// for (iI = 0; iI < NUM_HIMALAYA_REGIONS; iI++)
			// {
				// if (regionMap[EARTH_Y -1 -iY][iX] == himalayaRegions[iI])
				// {
					// if (!plotSorenINLINE(iX, iY)->isWater())
						// plotSorenINLINE(iX, iY)->setArea(iranID);
				// }
	...
	// {
		...
		// {
			...
			// {
				...
				// {
					// if (!plotSorenINLINE(iX, iY)->isWater())
						// plotSorenINLINE(iX, iY)->setArea(hindustanID);
				// }
			// }
		// }
	// }
	...
	for (iX = 0; iX < EARTH_X; iX++)
	{
		...
		for (iY = 0; iY < EARTH_Y; iY++)
		{
			if (regionMap[EARTH_Y -1 -iY][iX] == 31)
			{
				if (!plotSorenINLINE(iX, iY)->isWater())
					plotSorenINLINE(iX, iY)->setArea(bahrainID);
			}
		}
	}
	...
	for (iX = 0; iX < EARTH_X; iX++)
	{
		...
		for (iY = 0; iY < EARTH_Y; iY++)
		{
			if (regionMap[EARTH_Y -1 -iY][iX] == 32)
			{
				if (!plotSorenINLINE(iX, iY)->isWater())
					plotSorenINLINE(iX, iY)->setArea(mahraID);
			}
		}
	}
