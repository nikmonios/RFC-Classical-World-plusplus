...
void CvGame::initDiplomacy()
{
	...
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		...
		if (GET_TEAM((TeamTypes)iI).isBarbarian()) // edead: disable auto-war with minor civs
		{
...
void CvGame::update()
{
	...
	if (!gDLL->GetWorldBuilderMode() || isInAdvancedStart())
	{
		...
		if (getTurnSlice() == 0)
		{
			...
			// srpt leaderhead project this one doesn't matter, ActivePlayer will never be a respawn
			if ((getGameTurn() > 0) && !(getGameTurn() < getGameTurnForYear(startingYear[getActivePlayer()], getStartYear(), getCalendar(), getGameSpeedType())))
			{
				gDLL->getEngineIFace()->AutoSave(true);
			}
			// edead: end
		...
		// edead: start Rhye's AIAutoPlay
		// srpt leaderhead project this one doesn't matter, ActivePlayer will never be a respawn
		if (startingYear[getActivePlayer()] > GC.getGameINLINE().getStartYear())
		{
			if (getGameTurn() == 0)
				setAIAutoPlay(1);
			else if (getGameTurn() <= getGameTurnForYear(startingYear[getActivePlayer()], getStartYear(), getCalendar(), getGameSpeedType()))
...
//Speed: End Modify
		...
		GET_PLAYER(getActivePlayer()).initUnit(((UnitTypes)0), CATAPULT_X, CATAPULT_Y); //Rhye (catapult)
	}
}
...
void CvGame::setAIAutoPlayCatapult(int iNewValue)
{
	...
	if (iOldValue != iNewValue)
	{
		...
		if ((iOldValue == 0) && (getAIAutoPlay() > 0))
		{
			CvPlot* pPlot = GC.getMapINLINE().plotINLINE(CATAPULT_X, CATAPULT_Y);
			if (pPlot->isUnit()) {
				GC.getMapINLINE().plotINLINE(CATAPULT_X, CATAPULT_Y)->getUnitByIndex(0)->kill(false);
				for (int iI = 0; iI < MAX_PLAYERS; iI++)
				{
					if (GET_PLAYER((PlayerTypes)iI).isHuman())
					{
						GC.getMapINLINE().plotINLINE(CATAPULT_X, CATAPULT_Y-1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(CATAPULT_X, CATAPULT_Y)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(CATAPULT_X, CATAPULT_Y+1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(CATAPULT_X+1, CATAPULT_Y-1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(CATAPULT_X+1, CATAPULT_Y)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(CATAPULT_X+1, CATAPULT_Y+1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(CATAPULT_X-1, CATAPULT_Y-1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(CATAPULT_X-1, CATAPULT_Y)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(CATAPULT_X-1, CATAPULT_Y+1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
...
void CvGame::setWinner(TeamTypes eNewWinner, VictoryTypes eNewVictory)
{
	...
	if ((getWinner() != eNewWinner) || (getVictory() != eNewVictory))
	{
		...
		if (getVictory() != NO_VICTORY)
		{
			...
			{
				//szBuffer = gDLL->getText("TXT_KEY_GAME_WON", GET_TEAM(getWinner()).getName().GetCString(), GC.getVictoryInfo(getVictory()).getTextKeyWide()); // edead
				szBuffer = gDLL->getText("TXT_KEY_GAME_WON", GET_PLAYER((PlayerTypes)getWinner()).getCivilizationDescription(), GC.getVictoryInfo(getVictory()).getTextKeyWide()); // edead
...
void CvGame::setHolyCity(ReligionTypes eIndex, CvCity* pNewValue, bool bAnnounce)
{
	...
	if (pOldValue != pNewValue)
	{
		...
		if (getHolyCity(eIndex) != NULL)
		{
			...
			// srpt no announcements during setup
			if (GET_PLAYER((PlayerTypes)ANTIGONIDS).isPlayable())
			{
				if (eIndex == HELLENISM || eIndex == JAINISM || eIndex == CONFUCIANISM || eIndex == TAOISM)
				{
					bAnnounce = false;
				}
			}
...
void CvGame::doTurn()
{
	...
	//createBarbarianCities(); // edead: disabled (python only)

	//createBarbarianUnits(); // edead: disabled
...
/*************************************************************************************************/
	...
	// edead: disable autosave during autoplay
	// srpt leaderhead project this one doesn't matter, ActivePlayer will never be a respawn
	if (!GC.getDefineINT("DISABLE_AUTOSAVES_DURING_AUTOPLAY") || ((getGameTurn() > 0) && !(getGameTurn() < getGameTurnForYear(startingYear[getActivePlayer()], getStartYear(), getCalendar(), getGameSpeedType()))))
	{
		...
		if (GC.getVictoryInfo(eVictory).getReligionPercent() > 0)
		{
			...
			if (getNumCivCities() > (countCivPlayersAlive() * 2))
			{
				for (int iK = 0; iK < GC.getNumReligionInfos(); iK++)
				{
					...
					{
						// edead: start
						/*int iReligionPercent = GC.getVictoryInfo(eVictory).getReligionPercent();
						if (iK == SUNNI) iReligionPercent += 10;
						if (iK == ORTHODOXY) iReligionPercent += 5;
						if (iK == CATHOLICISM) iReligionPercent -= 5;
						if (calculateReligionPercent((ReligionTypes)iK) >= iReligionPercent)*/ // srpt
						if (calculateReligionPercent((ReligionTypes)iK) >= GC.getVictoryInfo(eVictory).getReligionPercent())// srpt uncomment
						// edead: end
						{
							bFound = true;
							break;
	...
	for (iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		...
		if (player.isHuman())
		{
			...
			if (NULL != pInfo)
			{
				...
				{
					//pInfo->setText(gDLL->getText("TXT_KEY_GAME_WON", GET_TEAM(getWinner()).getName().GetCString(), GC.getVictoryInfo(getVictory()).getTextKeyWide())); // edead
					pInfo->setText(gDLL->getText("TXT_KEY_GAME_WON", GET_PLAYER((PlayerTypes)getWinner()).getCivilizationDescription(), GC.getVictoryInfo(getVictory()).getTextKeyWide())); // edead
				}
				else
				{
					pInfo->setText(gDLL->getText("TXT_KEY_MISC_DEFEAT2"));
				}
...
CvWString CvGame::getRegionName(int i, bool bTooltip) const
{
	char szBuffer[20];
	CvWString szResult;
	if (bTooltip)
		sprintf(szBuffer, "TXT_KEY_REGION_TOOLTIP_%d", i);
	else
		sprintf(szBuffer, "TXT_KEY_REGION_%d", i);
