...
/************************************************************************************************/
									if ((pLoopUnit->getDomainType() != DOMAIN_LAND) || pLoopUnit->plot()->plotCount(PUF_isMilitaryHappiness, -1, -1, getID()) > 1)
									{
										...
										if ((calculateUnitCost() > 0) && (!AI_getAnyPlotDanger( pLoopUnit->plot(), 2, false))) // srpt
/************************************************************************************************/
/* BETTER_BTS_AI_MOD                       END                                                  */
/************************************************************************************************/	
										{
...
void CvPlayerAI::AI_conquerCity(CvCity* pCity)
{
	...
	if (GC.getGameINLINE().getSorenRandNum(100, "AI Massacre Disbelievers") < massacreProb[getID()])
	{
		bMassacre = true;
	}
	// edead: end
...
int CvPlayerAI::AI_foundValue(int iX, int iY, int iMinRivalRange, bool bStartingLoc) const
{
	...
	if (getID() < NUM_MAJOR_PLAYERS)
	{
		...
		iValue *= settlersMaps[(GET_PLAYER(getID()).getLeaderType())][EARTH_Y - 1 - tempY][tempX];
		iValue /= 100;

		if (settlersMaps[(GET_PLAYER(getID()).getLeaderType())][EARTH_Y - 1 - tempY][tempX] <= 60)
			iValue /= 2;
		if (settlersMaps[(GET_PLAYER(getID()).getLeaderType())][EARTH_Y - 1 - tempY][tempX] <= 3)
			iValue = 0;
...
int CvPlayerAI::AI_targetCityValue(CvCity* pCity, bool bRandomize, bool bIgnoreAttackers) const
{
	...
	if (pCity->isCoastal(GC.getMIN_WATER_SIZE_FOR_OCEAN()))
	{
		...
		if (getID() < NUM_MAJOR_PLAYERS)
		{
			iValue += warCoastalModifier[getID()];
		}
		// edead: end
	}
	...
	if (settlersMaps[(GET_PLAYER(getID()).getLeaderType())][EARTH_Y - 1 - pCity->plot()->getY_INLINE()][pCity->plot()->getX_INLINE()] <= 3)
		iValue = iValue/3 - 4;
	else if (settlersMaps[(GET_PLAYER(getID()).getLeaderType())][EARTH_Y - 1 - pCity->plot()->getY_INLINE()][pCity->plot()->getX_INLINE()] <= 20)
		iValue -= 3;
	else if (settlersMaps[(GET_PLAYER(getID()).getLeaderType())][EARTH_Y - 1 - pCity->plot()->getY_INLINE()][pCity->plot()->getX_INLINE()] >= 500) //500-700
		iValue = 2 + iValue*3;
	else if (settlersMaps[(GET_PLAYER(getID()).getLeaderType())][EARTH_Y - 1 - pCity->plot()->getY_INLINE()][pCity->plot()->getX_INLINE()] >= 300) //300-400
		iValue = 2 + iValue*2;
	else if (settlersMaps[(GET_PLAYER(getID()).getLeaderType())][EARTH_Y - 1 - pCity->plot()->getY_INLINE()][pCity->plot()->getX_INLINE()] >= 150) //150-200
		iValue += 3;
	else if (settlersMaps[(GET_PLAYER(getID()).getLeaderType())][EARTH_Y - 1 - pCity->plot()->getY_INLINE()][pCity->plot()->getX_INLINE()] >= 40) //40-60-90
		iValue += 1;

	// edead: modified a bit so that the AI is less interested in foreign minors
	if (pCity->getOwner() >= NUM_MAJOR_PLAYERS)
	{
		if (settlersMaps[(GET_PLAYER(getID()).getLeaderType())][EARTH_Y - 1 - pCity->plot()->getY_INLINE()][pCity->plot()->getX_INLINE()] < 40)
			iValue -= 2;
	}
	...
	//{
		//if ((pCity->getOwner() == AXUM) || (pCity->getOwner() == KUSHANS))
		//{
			//iValue /= 3;
		//}
	//}//srpt
...
/************************************************************************************************/
								...
								case CARTHAGE:
									...
									break;
								case MAURYANS:
									if (iI == MONASTICISM || iI == CROP_ROTATION || iI == HORTICULTURE) {
										iValue *= 2;
									}
									if (iI == ELEPHANT_TRAINING) {
										iValue *= 4;
									...
									break;
								case SATAVAHANA:
									if (iI == MARKSMANSHIP || iI == MONASTICISM || iI == STATE_RELIGION) {
										iValue *= 2;
									}
									if (iI == MILITARY_DRILL || iI == SCALE_ARMOR) {
										iValue /= 2;
									}
									if (iI == ELEPHANT_TRAINING) {
										iValue *= 4;
									...
									break;
								case PANDYANS:
									if (iI == HORTICULTURE || iI == METAL_CASTING || iI == STEEL_WORKING) {
										iValue *= 3;
									}
									break;
								case KALINKA:
									...
									break;
								case GUPTA:
									if (iI == AESTHETICS || iI == PATRONAGE || iI == EDUCATION) {
										iValue *= 2;
									}
									if (iI == ELEPHANT_TRAINING) {
										iValue *= 4;
									...
									break;
								case ARMENIA:
									if (iI == STATE_RELIGION || iI == THEOLOGY || iI == HORSE_ARCHERY) {
										iValue *= 2;
									}
								...
								case MACCABEES:
									...
									break;
								case BACTRIA:
									if (iI == MARKSMANSHIP || iI == CAVALRY_TACTICS) {
										iValue *= 2;
									}
									if (iI == FISHING || iI == SAILING || iI == NAVAL_WARFARE) {
										iValue /= 3;
									...
									break;
								case PARTHIA:
									if (iI == SCALE_ARMOR || iI == CAVALRY_TACTICS || iI == MARKSMANSHIP) {
										iValue *= 2;
									}
									if (iI == FISHING || iI == SAILING || iI == NAVAL_WARFARE) {
										iValue /= 2;
								...
								case DACIA:
									...
									break;
								case GOGURYEO:
									if (iI == CROP_ROTATION || iI == MILITARY_DRILL) {
										iValue /= 2;
									}
									break;
								case KUSHANS:
									if (iI == MONASTICISM || iI == THE_STIRRUP) {
										iValue /= 2;
									}
									if (iI == FISHING || iI == SAILING || iI == NAVAL_WARFARE) {
										iValue /= 3;
									...
									break;
								case AXUM:
									if (iI == STERN_RUDDER || iI == NAVIGATION) {
										iValue *= 2;
									}
									break;
								case FUNAN:
									if (iI == CROP_ROTATION || iI == HORTICULTURE) {
										iValue *= 2;
									}
									break;
								case SASSANIDS:
									if (iI == MYSTICISM || iI == THE_STIRRUP || iI == STEEL_WORKING) {
										iValue *= 2;
									}
									break;
								case YAMATO:
									if (iI == STEEL_WORKING || iI == MARKSMANSHIP || iI == VASSALAGE) {
										iValue *= 2;
									}
									break;
								case JIN:
									...
									break;
								case FRANKS:
									if (iI == STATE_RELIGION || iI == THE_STIRRUP) {
										iValue *= 2;
									}
									break;
								case SRIVAJAYA:
									if (iI == STERN_RUDDER || iI == MARKSMANSHIP) {
										iValue *= 2;
									}
									break;
								case TANG:
...
int CvPlayerAI::AI_getSameReligionAttitude(PlayerTypes ePlayer) const
{
	...
	//if (((getStateReligion() == CATHOLICISM) && (GET_PLAYER(ePlayer).getStateReligion() == ORTHODOXY)) || ((getStateReligion() == ORTHODOXY) && (GET_PLAYER(ePlayer).getStateReligion() == CATHOLICISM)))
	//{
		//bSameReligion = true;
	//}
	
	if ((getStateReligion() != NO_RELIGION) && (getStateReligion() == GET_PLAYER(ePlayer).getStateReligion()))//srpt uncomment
	//if (bSameReligion)//srpt
	// edead: end
	{
...
int CvPlayerAI::AI_getDifferentReligionAttitude(PlayerTypes ePlayer) const
{
	...
	//if (((getStateReligion() == SUNNI) && (GET_PLAYER(ePlayer).getStateReligion() == SHIA)) || ((getStateReligion() == SHIA) && (GET_PLAYER(ePlayer).getStateReligion() == SUNNI)))
	//{
		//iAttitude /= 2;
	//}
...
int CvPlayerAI::AI_bonusTradeVal(BonusTypes eBonus, PlayerTypes ePlayer, int iChange) const
{
	...
	// srpt  note to self put religion bonus adjustment here
	/*switch (getID()) {
		case QIN:
		case HAN:
		case JIN:
		case TANG:
			if ((eBonus == BONUS_SPICES) || (eBonus == BONUS_CINNAMON) || (eBonus == BONUS_GEMS) || (eBonus == BONUS_SILVER) || (eBonus == BONUS_GOLD)) 
			{
			iValue *= 2;
			iValue /= 3;
			}
	...
	//srpt start
	if (eBonus == BONUS_JUDAISM)
	{
		if ((GET_PLAYER(ePlayer).getHasReligionCount((ReligionTypes)0) < 1))
		{
			iValue = 0;
		}
	}
	if (eBonus == BONUS_ZOROASTRIANISM)
	{
		if ((GET_PLAYER(ePlayer).getHasReligionCount((ReligionTypes)1) < 1))
		{
			iValue = 0;
		}
	}
	if (eBonus == BONUS_HINDUISM)
	{
		if ((GET_PLAYER(ePlayer).getHasReligionCount((ReligionTypes)2) < 1))
		{
			iValue = 0;
		}
	}
	if (eBonus == BONUS_BUDDHISM)
	{
		if ((GET_PLAYER(ePlayer).getHasReligionCount((ReligionTypes)3) < 1))
		{
			iValue = 0;
		}
	}
	if (eBonus == BONUS_CONFUCIANISM)
	{
		if ((GET_PLAYER(ePlayer).getHasReligionCount((ReligionTypes)4) < 1))
		{
			iValue = 0;
		}
	}
	if (eBonus == BONUS_HELLENISM)
	{
		if ((GET_PLAYER(ePlayer).getHasReligionCount((ReligionTypes)5) < 1))
		{
			iValue = 0;
		}
	}
	if (eBonus == BONUS_TAOISM)
	{
		if ((GET_PLAYER(ePlayer).getHasReligionCount((ReligionTypes)6) < 1))
		{
			iValue = 0;
		}
	}
	if (eBonus == BONUS_JAINISM)
	{
		if ((GET_PLAYER(ePlayer).getHasReligionCount((ReligionTypes)7) < 1))
		{
			iValue = 0;
		}
	}
	if (eBonus == BONUS_CHRISTIANITY)
	{
		if ((GET_PLAYER(ePlayer).getHasReligionCount((ReligionTypes)8) < 1))
		{
			iValue = 0;
		}
	}
	if (eBonus == BONUS_MANICHAEISM)
	{
		if ((GET_PLAYER(ePlayer).getHasReligionCount((ReligionTypes)9) < 1))
		{
			iValue = 0;
		}
	}
	if (eBonus == BONUS_ISLAM)
	{
		if ((GET_PLAYER(ePlayer).getHasReligionCount((ReligionTypes)10) < 1))
		{
			iValue = 0;
		}
	}
	...
	//{
		//if ((GET_PLAYER(ePlayer).getStateReligion() == SUNNI) || (GET_PLAYER(ePlayer).getStateReligion() == SHIA))
		//{
		//iValue *= 2;
		//iValue /= 3;
		//}
...
DenialTypes CvPlayerAI::AI_bonusTrade(BonusTypes eBonus, PlayerTypes ePlayer) const
{
	...
	{
		return (GET_PLAYER(ePlayer).isHuman() ? DENIAL_NO_GAIN : DENIAL_NO_GAIN);
	}
	...
	//srpt AI civs treat corporation resources as strategic rather than refusing to trade them
...
int CvPlayerAI::AI_cityTradeVal(CvCity* pCity) const
{
	...
	if (GET_PLAYER(pCity->getOwnerINLINE()).getNumCities() > 12) 
		if (settlersMaps[(GET_PLAYER(pCity->getOwnerINLINE()).getLeaderType())][EARTH_Y -1 - pCity->getY()][pCity->getX()] < 400) { //not in core area
			iValue *= 2;
			iValue /= 3;
		}
	//Rhye - end
...
/************************************************************************************************/	
	...
	iValue += -((kCivic.getDistanceMaintenanceModifier() * std::max(0, (getNumCities() - 3))) / 5); // srpt make it more attractive for large empires
...
/************************************************************************************************/
	...
	switch (GET_PLAYER(getID()).getLeaderType()) {
		case ANTIGONUS:
		case SELEUCUS:
			if (eCivic == CIVIC_SLAVERY || eCivic == CIVIC_TYRANNY)
			{
				iValue *= 2;
			}
			if (eCivic == CIVIC_TRIBAL_CUSTOM || eCivic == CIVIC_TRIBAL_LEVY)
			{
				iValue /= 4;
			}
			break;
		case PTOLEMY:
			if (eCivic == CIVIC_SLAVERY || eCivic == CIVIC_TYRANNY || eCivic == CIVIC_DYNASTIC_CULT)
			{
				iValue *= 2;
			}
			if (eCivic == CIVIC_TRIBAL_CUSTOM || eCivic == CIVIC_TRIBAL_LEVY || eCivic == CIVIC_PAGANISM)
			{
				iValue /= 4;
			}
			break;
		case HANNIBAL:
			...
			if (eCivic == CIVIC_TRIBAL_LEVY)
			{
				iValue /= 4;
			}
			break;
		case SHAPUR:
			if (eCivic == CIVIC_EMPIRE || eCivic == CIVIC_VASSALAGE || eCivic == CIVIC_SLAVERY || eCivic == CIVIC_STATE_RELIGION)
			{
				iValue *= 2;
			}
			break;
		case CAESAR:
			if (eCivic == CIVIC_EMPIRE || eCivic == CIVIC_BUREAUCRACY || eCivic == CIVIC_SLAVERY || eCivic == CIVIC_STATE_RELIGION)
			{
				iValue *= 2;
			}
			if (eCivic == CIVIC_TYRANNY || eCivic == CIVIC_MONARCHY || eCivic == CIVIC_TRIBAL_LEVY || eCivic == CIVIC_VASSALAGE || eCivic == CIVIC_OLIGARCHY || eCivic == CIVIC_PAGANISM)
			{
				iValue /= 4;
			}
			break;
		case JUSTINIAN:
			if (eCivic == CIVIC_EMPIRE || eCivic == CIVIC_BUREAUCRACY || eCivic == CIVIC_STATE_RELIGION || eCivic == CIVIC_SLAVERY)
			{
				iValue *= 2;
			}
			if (eCivic == CIVIC_TYRANNY || eCivic == CIVIC_MONARCHY || eCivic == CIVIC_VASSALAGE || eCivic == CIVIC_TRIBAL_LEVY || eCivic == CIVIC_DYNASTIC_CULT)
			{
				iValue = 0;
			}
			break;
		case WU:
			if (eCivic == CIVIC_EMPIRE || eCivic == CIVIC_BUREAUCRACY)
			{
				iValue *= 2;
			}
			if (eCivic == CIVIC_TYRANNY || eCivic == CIVIC_MONARCHY || eCivic == CIVIC_VASSALAGE)
			{
				iValue /= 4;
			}
			break;
		case QIN_SHI_HUANG:
			if (eCivic == CIVIC_DYNASTIC_CULT || eCivic == CIVIC_EMPIRE || eCivic == CIVIC_TYRANNY || eCivic == CIVIC_SLAVERY)
			{
				iValue *= 2;
			}
			if (eCivic == CIVIC_TRIBAL_LEVY || eCivic == CIVIC_TRIBAL_CUSTOM)
			{
				iValue /= 4;
			}
			break;
		...
		case PULAKESI:
			...
			if (eCivic == CIVIC_SLAVERY || eCivic == CIVIC_DYNASTIC_CULT)
			{
				iValue /= 4;
			}
			break;
...
int CvPlayerAI::AI_religionValue(ReligionTypes eReligion) const
{
	...
	/*// edead: start
	if ((eReligion == BUDDHISM) || (eReligion == JUDAISM) || (eReligion == ZOROASTRIANISM))
	{
		return 0;
	}*/ // srpt
	...
	switch (GET_PLAYER(getID()).getLeaderType()) {
		...
		case PTOLEMY:
			if (eReligion == HELLENISM)
				iValue += 10;
			else if (eReligion == CHRISTIANITY)
				return iValue;
			else
				return 0;
			break;
		...
		case ASOKA:
			if (eReligion == BUDDHISM)
				iValue += 2;
			else if ((eReligion == JAINISM) || (eReligion == HINDUISM))
				iValue /= 2;			
			else
				return 0;
		case PUSYAMITRA:
			if (eReligion == HINDUISM)
				iValue += 2;
			else if ((eReligion == JAINISM) || (eReligion == BUDDHISM))
				iValue /= 2;			
			else
				return 0;
			break;
		...
		case TIGRANES:
			if (eReligion == CHRISTIANITY)
				iValue += 10;
			else if (eReligion == ZOROASTRIANISM)
				return iValue;
			else
				return 0;
			break;
		...
		case TUBA_ABU:
			if (eReligion == JUDAISM)
				iValue += 10;
			else if ((eReligion == ISLAM) || (eReligion == CHRISTIANITY))
				return iValue;
			else
				return 0;
			break;
		case EZANA:
			if (eReligion == CHRISTIANITY)
				iValue += 10;
			else if ((eReligion == JUDAISM) || (eReligion == ISLAM))
				return iValue;
			else
				return 0;
			break;
		...
		case SAPADBIZES:
			if (eReligion == BUDDHISM)
				iValue += 10;
			else if ((eReligion == CHRISTIANITY) || (eReligion == ISLAM))
				return iValue;
			else
				return 0;
			break;
		...
		case NEDUNJ:
			if ((eReligion == JAINISM) || (eReligion == HINDUISM))
				iValue += 10;
		...
		case WU:
		case SIMA_YAN:
			if (eReligion == CONFUCIANISM)
				iValue += 10;
			else if ((eReligion == BUDDHISM) || (eReligion == TAOISM))
				iValue /= 2;
			else
				return 0;
			break;
		case TAIZONG:
			if (eReligion == BUDDHISM)
				iValue += 10;
			else if ((eReligion == CONFUCIANISM) || (eReligion == TAOISM))
				iValue /= 2;
			else
				return 0;
			break;
		...
		case PULAKESI:
			if (eReligion == HINDUISM)
				iValue += 10;
			else if ((eReligion == BUDDHISM) || (eReligion == JAINISM))
				return iValue;
			else
				return 0;
			break;
		...
		case DEMETRIUS:
			if ((eReligion == HELLENISM) || (eReligion == BUDDHISM))
				iValue += 10;
			else
				iValue /= 2;
			break;
		...
		case ARSACES:
			if (eReligion == ZOROASTRIANISM || (eReligion == HELLENISM))
				iValue += 50;
			else
				iValue = 0;
			break;
		case SHAPUR:
			if (eReligion == ZOROASTRIANISM)
				iValue += 50;
			else
				iValue = 0;
			break;
		...
		case GWANGGAETO:
			if ((eReligion == CONFUCIANISM) || (eReligion == BUDDHISM))
				iValue += 10;
			else
				return 0;
			break;
		...
		case KANISHKA:
			if (eReligion == BUDDHISM)
				iValue += 10;
			else if ((eReligion == HELLENISM) || (eReligion == HINDUISM))
				return iValue;
			else
				return 0;
			break;
...
{
	if (GET_TEAM(GET_PLAYER(ePlayer).getTeam()).getProjectCount((ProjectTypes)PROJECT_CALIPH) > 0)
	{
...
{
	/*if (GET_TEAM(GET_PLAYER(ePlayer).getTeam()).getProjectCount((ProjectTypes)PROJECT_SHARIF) > 0)
	{
		if ((getStateReligion() == SUNNI) || (getStateReligion() == SHIA))
		{
			return 1;
		}
	}
...
{
	/*if (GET_TEAM(GET_PLAYER(ePlayer).getTeam()).getProjectCount((ProjectTypes)PROJECT_RAJA) > 0)
	{
		if ((getStateReligion() == HINDUISM))
		{
			return 2;
		}
	}
...
void CvPlayerAI::read(FDataStreamBase* pStream)
{
	...
	pStream->Read(NUM_UNITAI_TYPES, m_aiUnitAIModifier); // edead
...
void CvPlayerAI::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(NUM_UNITAI_TYPES, m_aiUnitAIModifier); // edead
...
bool CvPlayerAI::AI_disbandUnit(int iExpThreshold, bool bObsolete)
{
	...
	// srpt leaderhead project respawns will be unprotected by this for now
	if (GC.getGameINLINE().getGameTurn() < getTurnForYear(startingYear[getID()])+10) 
	{
		return false;
	}
	// edead: end
...
void CvPlayerAI::AI_updateCitySites(int iMinFoundValueThreshold, int iMaxSites) const
{
	...
	while (iPass < iMaxSites)
	{
		...
		for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
		{
			...
			if (pLoopPlot->isRevealed(getTeam(), false))
			{
				if (settlersMaps[(GET_PLAYER(getID()).getLeaderType())][EARTH_Y -1 - pLoopPlot->getY()][pLoopPlot->getX()] >= 90) //Rhye
				{ //Rhye
