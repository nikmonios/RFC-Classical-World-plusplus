...
CvTeam::CvTeam()
{
	...
	// srpt start Techs allow specialists
...
/********************************************************************************/
// srpt start Techs allow specialists
int CvTeam::getSpecialistValidCount(SpecialistTypes eIndex) const
{
...
void CvTeam::changeSpecialistValidCount(SpecialistTypes eIndex, int iChange)
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < GC.getNumSpecialistInfos(), "eIndex is expected to be within maximum bounds (invalid Index)");
...
bool CvTeam::isSpecialistValid(SpecialistTypes eIndex) const
{
...
void CvTeam::doTurn()
{
	...
	if ((!GC.getGameINLINE().isOption(GAMEOPTION_NO_TECH_BROKERING)) || (getLeaderID() == EZANA))
	{
		for (iI = 0; iI < GC.getNumTechInfos(); iI++)
		{
...
bool CvTeam::canDeclareWar(TeamTypes eTeam) const
{
	...
	// srpt no far distant wars
	if (!isHuman())
	{
		if (eTeam <= NUM_MAJOR_PLAYERS)
		{
			if ((borders[(int)getID()][(int)eTeam]) == 5)
			{
				return false;
			}
		}
...
int CvTeam::getResearchCost(TechTypes eTech) const
{
	...
	if (getID() < NUM_MAJOR_PLAYERS) {		
		...
		if (GC.getGameINLINE().getGameTurn() >= getTurnForYear(startingYear[getID()])) { // srpt: this is here to prevent the mechanism from giving a huge research bonus to Rebel Rome/Byzantines, leaderhead project should be unaffected, other than respawns not getting the bonus
			if (GC.getGameINLINE().getGameTurn() <= getTurnForYear(startingYear[getID()]) + iTurnModifier) { // edead
				...
				int iAmount = (getTurnForYear(startingYear[getID()]) + iTurnModifier) - GC.getGameINLINE().getGameTurn();
				iAmount *= 100;
	...
	{
		bool bKnown = false;
		if ((GET_TEAM((TeamTypes)GOGURYEO).isHasTech(eTech)) && (GET_TEAM((TeamTypes)GOGURYEO).canContact((TeamTypes)YAMATO)))
		{
			bKnown = true;
		}
		if ((GET_TEAM((TeamTypes)JIN).isHasTech(eTech)) && (GET_TEAM((TeamTypes)JIN).canContact((TeamTypes)YAMATO)))
		{
			bKnown = true;
		}
		if ((GET_TEAM((TeamTypes)TANG).isHasTech(eTech)) && (GET_TEAM((TeamTypes)TANG).canContact((TeamTypes)YAMATO)))
		{
			bKnown = true;
		}
		if ((GET_TEAM((TeamTypes)HAN).isHasTech(eTech)) && (GET_TEAM((TeamTypes)HAN).canContact((TeamTypes)YAMATO)))
		{
			bKnown = true;
		}
		if ((GET_TEAM((TeamTypes)HAN).isHasTech(eTech)) && (GET_TEAM((TeamTypes)HAN).canContact((TeamTypes)YAMATO)))
		...
		if ((GET_TEAM((TeamTypes)QIN).isHasTech(eTech)) && (GET_TEAM((TeamTypes)QIN).canContact((TeamTypes)YAMATO)))
		{
			bKnown = true;
		}
		if (bKnown)
	...
	if (iChange != 0)
	{
		...
		if (iChange > 0)
		{
			...
			if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
			{
				...
				if (getID() == FUNAN) szTemp = gDLL->getText("TXT_KEY_LEADER_ZENGI"); // edead: stupid hack
				if (getID() == TOCHARIANS) szTemp = gDLL->getText("TXT_KEY_LEADER_AL_DAWLA"); // edead: stupid hack
				...
				for (iI = 0; iI < MAX_PLAYERS; iI++)
				{
					...
					// srpt leaderhead project there are no projects right now
					if ((GET_PLAYER((PlayerTypes)iI).isAlive()) && (GC.getGameINLINE().getGameTurn() > 1) && (GC.getGameINLINE().getGameTurn() >= getTurnForYear(startingYear[iI]))) // edead
					{
						...
						//gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_PROJECT_COMPLETED", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT")); // edead
						gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_FEAT_ACCOMPLISHED", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT")); // edead
			...
			if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
			{
				for (iI = 0; iI < MAX_PLAYERS; iI++)
				{
					if (GET_PLAYER((PlayerTypes)iI).isAlive())
					{
						...
						if (getID() == FUNAN) szTemp = gDLL->getText("TXT_KEY_LEADER_ZENGI"); // edead: stupid hack
						if (getID() == TOCHARIANS) szTemp = gDLL->getText("TXT_KEY_LEADER_AL_DAWLA"); // edead: stupid hack
						//szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_HAS_LOST_PROJECT", GET_PLAYER((PlayerTypes)getID()).getName(), GC.getProjectInfo(eIndex).getTextKeyWide());
						szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_HAS_LOST_PROJECT", szTemp.GetCString(), GC.getProjectInfo(eIndex).getTextKeyWide()); // edead
...
void CvTeam::processTech(TechTypes eTech, int iChange)
{
	...
	for (iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
	{
		changeSpecialistValidCount(((SpecialistTypes)iI), ((GC.getTechInfo(eTech).isSpecialistValid(iI)) ? iChange : 0));
	}
	// srpt end
