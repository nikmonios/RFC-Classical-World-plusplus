...
/************************************************************************************************/
	...
	if (!bDanger && (!hasActiveWorldWonder()) && (kPlayer.getNumCities() <= (getOwnerINLINE() == CARTHAGE ? 6 : 4)))
	{
...
BuildingTypes CvCityAI::AI_bestBuildingThreshold(int iFocusFlags, int iMaxTurns, int iMinThreshold, bool bAsync, AdvisorTypes eIgnoreAdvisor)
{
	...
	for (iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)  // srpt for each building class
	{
		if (!(GET_PLAYER(getOwnerINLINE()).isBuildingClassMaxedOut(((BuildingClassTypes)iI), GC.getBuildingClassInfo((BuildingClassTypes)iI).getExtraPlayerInstances())))  // srpt if you can build it
		{
			eLoopBuilding = ((BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(iI))); // srpt eBuilding is your version of the class

			if ((eLoopBuilding != NO_BUILDING) && (getNumBuilding(eLoopBuilding) < GC.getCITY_MAX_NUM_BUILDINGS())
			&&  (!isProductionAutomated() || !(isWorldWonderClass((BuildingClassTypes)iI) || isNationalWonderClass((BuildingClassTypes)iI)))) // srpt again can you build it and is it a wonder or national wonder
			{
				//don't build wonders?
							...
							if (iValue > 0)
							{
								...
								if (isWorldWonderClass((BuildingClassTypes)iI))
								{
									if (iProductionRank <= std::min(3, ((GET_PLAYER(getOwnerINLINE()).getNumCities() + 2) / 3)))
									{
										...
										if (iI == GREAT_LIGHTHOUSE && getX_INLINE() == 53 && getY_INLINE() == 35) {
											iTempValue *= 2;
										}
										if (iI == GREAT_LIBRARY && getX_INLINE() == 53 && getY_INLINE() == 35) {
											iTempValue *= 3;
											iTempValue /= 2;
										}
										if (iI == GREAT_WALL && getX_INLINE() == 134 && getY_INLINE() == 50) {
											iTempValue *= 2;
										}
										if (iI == TERRACOTTA && getX_INLINE() == 134 && getY_INLINE() == 50) {
											iTempValue *= 3;
											iTempValue /= 2;
										}
										if (iI == TRAJANS_COLUMN && getX_INLINE() == 35 && getY_INLINE() == 52) {
											iTempValue *= 2;
										}
										if (iI == COLLOSEUM && getX_INLINE() == 35 && getY_INLINE() == 52) {
											iTempValue *= 3;
											iTempValue /= 2;
										}
										if (iI == THEODOSIAN_WALLS && getX_INLINE() == 54 && getY_INLINE() == 53) {
											iTempValue *= 3;
										}
										...
										{
										case EGYPT:
											if (iI == GREAT_LIGHTHOUSE || iI == GREAT_LIBRARY) {
												iTempValue *= 2;
											}
											break;
										case QIN:
											if (iI == GREAT_WALL || iI == TERRACOTTA) {
												iTempValue *= 2;
											}
											break;
										case MAURYANS:
											if (iI == BUDDHIST_SHRINE) {
												iTempValue *= 2;
											}
											if (iI == HINDU_SHRINE) {
												iTempValue /= 2;
											...
											break;
										case ROME:
											if (iI == TRAJANS_COLUMN) {
												iTempValue *= 3;
											}
											if (iI == WALLS || iI == COURTHOUSE || iI == COLLOSEUM) {
												iTempValue *= 2;
											}
											if (GET_PLAYER(getOwnerINLINE()).getNumCities() < 10) {
												if (iI == INVASION) {
													iTempValue *= 3;
												}
											}
...
bool CvCityAI::AI_chooseUnit(UnitAITypes eUnitAI)
{
	...
	if (getOwnerINLINE() < NUM_MAJOR_PLAYERS)
	{
		...
		// }
		if (((eUnitAI == UNITAI_SPY) || (eUnitAI == UNITAI_MISSIONARY)) && (canTrain((UnitTypes)UNIT_PROSECUTOR)) && (NO_RELIGION != GET_PLAYER(getOwnerINLINE()).getStateReligion())) // edead
		{
			//GC.getGameINLINE().logMsg("   Step 2 ");
			// 3MiroProsecutions
			if ((GET_PLAYER(getOwnerINLINE()).getUnitClassCount((UnitClassTypes)UNIT_PROSECUTOR_CLASS) < 1) && (GC.getGameINLINE().getSorenRandNum(100," Shall we build a Prosecutor") < buildProsecutorProb[getOwnerINLINE()]) ) // changed to array - edead
			{
				// edead: check whether the persecution is actually needed
				bool bFound = false;
				for (int i = 0; i < GC.getNumReligionInfos(); i++)
				{
					if ((GET_PLAYER(getOwnerINLINE()).getStateReligion() != (ReligionTypes)i) && (GET_PLAYER(getOwnerINLINE()).getHasReligionCount((ReligionTypes)i) > 0))
					{
