...
DenialTypes CvTeamAI::AI_techTrade(TechTypes eTech, TeamTypes eTeam) const
{
	...
	// srpt leaderhead project this won't matter much
	if (GC.getGameINLINE().getGameTurn() <= getTurnForYear(startingYear[getID()])+2) // edead
	{
		return DENIAL_TECH_MONOPOLY;
	}
	//if (GC.getGameINLINE().getGameTurn() <= startingTurn[eTeam]+2)
	if (GC.getGameINLINE().getGameTurn() <= getTurnForYear(startingYear[eTeam])+2) // edead
	{
...
DenialTypes CvTeamAI::AI_surrenderTrade(TeamTypes eTeam, int iPowerMultiplier) const
{
	...
	// srpt leaderhead project respawns do not have the flip delay
	if (GC.getGameINLINE().getGameTurn() <= getTurnForYear(startingYear[getID()])+2)
	{
		return DENIAL_JOKING;
	}
	// edead: end
...
/************************************************************************************************/
	...
	if (!GET_TEAM(eTeam).isParent(getID()))
	{
		...
		for (std::vector<PlayerTypes>::const_iterator iter = m_aePlayerMembers.begin(); iter != m_aePlayerMembers.end(); ++iter)
		{
			...
			if ((int(*iter) == KUSHANS) || (int(*iter) == AXUM)) 
			{
				if (GET_PLAYER(kMasterTeam.getLeaderID()).getStateReligion() == CATHOLICISM)
					iPersonalityModifier -= 5;
				else if (GET_PLAYER(kMasterTeam.getLeaderID()).getStateReligion() == ORTHODOXY)
