...
bool CvPlayer::canTrain(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	// srpt "Or" tech prereqs
	
	bool bHasOrTechPrereq = false;
	bool bHasOrTech = false;
	...
	if ((bHasOrTech == false) && (bHasOrTechPrereq == true))
	{
		return false;
	}
	// srpt end
...
int CvPlayer::getProductionNeeded(UnitTypes eUnit) const
{
	...
	// srpt start unit production supply
	iProductionNeeded *= (calculateUnitCost() * 2) + 100;
	iProductionNeeded /= 100;
	// srpt end
...
int CvPlayer::calculateUnitCost(int& iFreeUnits, int& iFreeMilitaryUnits, int& iPaidUnits, int& iPaidMilitaryUnits, int& iBaseUnitCost, int& iMilitaryCost, int& iExtraCost) const
{
	...
	if (isMinorCiv())
	{
		iFreeMilitaryUnits += getNumCities() *3; // srpt
	}
	// edead: end
...
/************************************************************************************************/	
	...
	//srpt - start UP
	if ((GET_PLAYER(getID()).getLeaderType()) == ASOKA)
	{
		if ((eCivic == 22))
		{
			return true;
...
{
	if (getHasReligionCount(eReligion) * 2 < getNumCities()) // srpt religion must be present in half your cities
	{
		return false;
	}
...
int CvPlayer::getGoldenAgeLength() const	 
{
	//c-addicted: Gupta UP
	if (getID() == GUPTA) {
		return (150 * GC.getGameINLINE().goldenAgeLength() * std::max(0, 100 + getGoldenAgeModifier())) / 10000;
	}else{
...
{
	// edead: start UP: Philosophy (Egypt)
	if ((getID() == EGYPT) && (getCurrentEra() < 2)) {
		return (m_iGreatPeopleRateModifier + 50);
	}
	// edead: end
		...
		else
		{
			...
			if (GC.getGameINLINE().getElapsedGameTurns() > 0)
			{
				...
				if (!isBarbarian() && !isMinorCiv()) //Rhye
				{
					...
					{
						//if (GET_PLAYER((PlayerTypes)iI).isAlive())
						// srpt leaderhead project this one doesn't matter because respawns will always be after the first birth
		...
		if (!bKill)
		{
			if (!isBarbarian())
			{
				if (getNumCities() == 0 && getAdvancedStartPoints() < 0)
				{
					...
					// srpt leaderhead project again this probably won't matter
					if ((getNumUnits() == 0) || (isFoundedFirstCity() && (GC.getGameINLINE().getGameTurn() > getTurnForYear(startingYear[getID()]) + 10))) // edead
					{
						bKill = true;
					}
				}
...
int CvPlayer::getSettlersMaps(int y, int x)
{
	...
	//return settlersMaps[getLeaderType()][y][x]; // srpt
	return settlersMaps[getCivilizationType()][y][x];
}
...
void CvPlayer::createGreatPeople(UnitTypes eGreatPersonUnit, bool bIncrementThreshold, bool bIncrementExperience, int iX, int iY)
{
	...
	// edead: start
	gDLL->getInterfaceIFace()->addMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), szReplayMessage, "AS2D_UNIT_GREATPEOPLE", MESSAGE_TYPE_MAJOR_EVENT, pGreatPeopleUnit->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_UNIT_TEXT"), iX, iY, true, true);
	// for (int iI = 0; iI < MAX_PLAYERS; iI++)
	// {
		// if (GET_PLAYER((PlayerTypes)iI).isAlive())
		// {
...
bool CvPlayer::splitEmpire(int iAreaId)
{
	...
	//if (!bPlayerExists)
	if (eNewPlayer >= INDEPENDENT && eNewPlayer <= INDEPENDENT3)
	//Rhye - end
	{
		//Rhye - start comment
		/*
...
bool CvPlayer::isHasBuilding(BuildingTypes eIndex) // srpt
{
    return (countNumBuildings(eIndex) > 0);
}

// edead: start
int CvPlayer::getBorders(int iPlayer) const
{
	if (iPlayer < 0 || iPlayer >= NUM_MAJOR_PLAYERS) 
	{
		return -1;
	}
	
	return borders[getID()][iPlayer];
}
// edead: end