...
void CvCity::processBuilding(BuildingTypes eBuilding, int iChange, bool bObsolete)
{
	...
	if (!(GET_TEAM(getTeam()).isObsoleteBuilding(eBuilding)) || bObsolete)
	{
		...
		// srpt start Specialist bonus from Civics
		...
		// srpt end Specialist bonus from Civics
...
int CvCity::getOvercrowdingPercentAnger(int iExtra) const
{
	...
	iOvercrowding = (getPopulation() + iExtra); // srpt 
	//iOvercrowding = (((getPopulation() *2) -1) + iExtra); // srpt non-linear unhappiness from population
...
int CvCity::getHurryPercentAnger(int iExtra) const
{
	...
	/*if (getOwnerINLINE() == QIN) iAnger /= 2;  // srpt Qin UP // no longer part of the UP  */
	return iAnger;
}
...
int CvCity::unhappyLevel(int iExtra) const
{
	...
	if (!isNoUnhappiness())
	{
		...
		iUnhappiness = ((iAngerPercent * (getPopulation() + iExtra)) / GC.getPERCENT_ANGER_DIVISOR()); // srpt
		//iUnhappiness = ((iAngerPercent * (((getPopulation() *2) -1) + iExtra)) / GC.getPERCENT_ANGER_DIVISOR());
...
int CvCity::unhealthyPopulation(bool bNoAngry, int iExtra) const
{
	...
	return std::max(0, ((getPopulation() + iExtra - ((bNoAngry) ? angryPopulation(iExtra) : 0)))); // srpt
	//return std::max(0, ((((getPopulation() *2) -1) + iExtra - ((bNoAngry) ? angryPopulation(iExtra) : 0))));
}
...
int CvCity::foodDifference(bool bBottom) const
{
	...
	else
	{
		for (pLoopCity = GET_PLAYER(getOwnerINLINE()).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(getOwnerINLINE()).nextCity(&iLoop))
		{
			if (pLoopCity != this)
			{
				if (pLoopCity->getPopulation() > 2)
				{
					int iDistance = plotDistance(pLoopCity->getX_INLINE(), pLoopCity->getY_INLINE(), getX_INLINE(), getY_INLINE());
					iDifference += (pLoopCity->foodDifference()) - iDistance / 7;
				}
			}
		}
	}
...
void CvCity::updateFeatureHealth()
{
	...
	for (iI = 0; iI < NUM_CITY_PLOTS; iI++)
	{
		...
		if (pLoopPlot != NULL)
		{
			...
			{
				// srpt Funan UP
				if (((GET_PLAYER(getOwnerINLINE()).getLeaderType()) != LIEUYE) && (eFeature != JUNGLE))
				{
...
int CvCity::getReligionBadHappiness() const
{
	...
	return (getOwnerINLINE() == KUSHANS ? (m_iReligionBadHappiness / 2) : m_iReligionBadHappiness); // edead - UP: Tolerance
}
...
void CvCity::updateCultureLevel(bool bUpdatePlotGroups)
{
	...
	if (((getOwnerINLINE() >= NUM_MAJOR_PLAYERS)) && (eCultureLevel > ((CultureLevelTypes)2)))
	{
...
int CvCity::getBaseCommerceRateTimes100(CommerceTypes eIndex) const
{
	...
	// srpt Chalukyan UP
	if ((eIndex == COMMERCE_CULTURE) && (getOwnerINLINE() == CHALUKYANS))
	{
...
int CvCity::getProductionToCommerceModifier(CommerceTypes eIndex) const										 
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < NUM_COMMERCE_TYPES, "eIndex expected to be < NUM_COMMERCE_TYPES");
	// srpt: start Chalukyan UP
	if (getOwnerINLINE() == CHALUKYANS) 
	{
		if (eIndex == COMMERCE_CULTURE)
		{
			return (m_aiProductionToCommerceModifier[eIndex] * 200 / 100);
		}
	}
	// edead: end
	...
	if (getNumBuilding(eBuilding) > 0)
	{
		...
		if (!(kBuilding.isCommerceChangeOriginalOwner(eIndex)) || (getBuildingOriginalOwner(eBuilding) == getOwnerINLINE()))
		{
			...
			if (getNumActiveBuilding(eBuilding) > 0)
			{
				...
				{
					// srpt start state religion shrine adjustment
					if (GC.getBuildingInfo(eBuilding).getReligionType() == GET_PLAYER(getOwnerINLINE()).getStateReligion())
					{
						iCommerce += std::min(20, ((GC.getReligionInfo((ReligionTypes)(GC.getBuildingInfo(eBuilding).getGlobalReligionCommerce())).getGlobalReligionCommerce(eIndex) * GC.getGameINLINE().countReligionLevels((ReligionTypes)(GC.getBuildingInfo(eBuilding).getGlobalReligionCommerce()))) / 2 ));
					}
					else
					{
						...
						iCommerce += std::min(10, ((GC.getReligionInfo((ReligionTypes)(GC.getBuildingInfo(eBuilding).getGlobalReligionCommerce())).getGlobalReligionCommerce(eIndex) * GC.getGameINLINE().countReligionLevels((ReligionTypes)(GC.getBuildingInfo(eBuilding).getGlobalReligionCommerce()))) / 3 )); // srpt added divisor
						//iCommerce /= 2; // srpt
						//Rhye - end
					}
				}
...
int CvCity::getReligionCommerceByReligion(CommerceTypes eIndex, ReligionTypes eReligion) const
{
	...
	//{
	if (isHasReligion(eReligion)) // srpt all religions always generate culture, state religion X2
	{
...
int CvCity::getRevoltTestProbability() const
{
	...
	{
		case KUSHANS: // edead: UP: Tolerance
			result /= 2;
			break;
		case INDEPENDENT:
		case INDEPENDENT2:
			...
			if (bFirst)
			{
				...
				if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
				{
					...
					if (isWorldWonderClass((BuildingClassTypes)(GC.getBuildingInfo(eIndex).getBuildingClassType())))
					{
						...
						{
							//if (GET_PLAYER((PlayerTypes)iI).isAlive())
							if ((GET_PLAYER((PlayerTypes)iI).isAlive()) && (GC.getGameINLINE().getGameTurn() >= getTurnForYear(startingYear[iI]) + 1)) // edead srpt leaderhead project this seems to refer only to the human player
							{
								if (isRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false))
								{
...
{
	return; // srpt now handled exclusively in python
}
/*{
	CvCity* pLoopCity;
	int iRandThreshold;
...
//Speed: End Modify
	//Rhye - end
// // srpt religions will now spread randomly to cities that already have a religion, chance diminshes for each religion present
	//if (getReligionCount() <= 0)
	//{
	for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
	{
					...
					if (GET_PLAYER((PlayerTypes)iJ).isAlive())
					{
						for (pLoopCity = GET_PLAYER((PlayerTypes)iJ).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER((PlayerTypes)iJ).nextCity(&iLoop))
						{
							if (pLoopCity->isConnectedTo(this))
							{
								...
								//edead - start Buddhism decline//srpt switched to hellenism and zoroastrianism with increasing buddhism spread to east asia
								if (iI == HELLENISM)
								{
									if (GC.getGameINLINE().getGameTurnYear() >= 30)
									{
										int iHellenismModifier = std::max(1, (100 - GC.getGameINLINE().getGameTurnYear()) / 2);
										iSpreadFactor *= iHellenismModifier;
										iSpreadFactor /= 100;
									}
								}
								if (iI == ZOROASTRIANISM)
								{
									if (GC.getGameINLINE().getGameTurnYear() >= 500)
									{
										int iZoroastrianismModifier = std::max(1, (400 - GC.getGameINLINE().getGameTurnYear()) / 2);
										iSpreadFactor *= iZoroastrianismModifier;
										iSpreadFactor /= 100;
									}
								}
								if (iI == BUDDHISM)
								{
									if ((getOwnerINLINE() == QIN) || (getOwnerINLINE() == HAN) || (getOwnerINLINE() == JIN) || (getOwnerINLINE() == GOGURYEO) || (getOwnerINLINE() == TANG) || (getOwnerINLINE() == YAMATO))
									{
										if (GC.getGameINLINE().getGameTurnYear() >= 100)
										{
											int iBuddhismModifier = std::max(1, (900 + GC.getGameINLINE().getGameTurnYear()));
								...
								if (iSpread > 0) 
								{
									...
									iSpread /= ((getReligionCount()) * (getReligionCount()) * (getReligionCount())) + 5; // srpt uncomment
