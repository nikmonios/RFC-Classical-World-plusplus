...
void CvUnit::reset(int iID, UnitTypes eUnit, PlayerTypes eOwner, bool bConstructorCall)
{
	...
	m_szName.clear();
	m_szArtDefineTag = ""; // edead // srpt indy unit art
	m_szScriptData ="";
...
void CvUnit::kill(bool bDelay, PlayerTypes ePlayer)
{
	...
	if (ePlayer != NO_PLAYER)
	{
		...
		if (NO_UNIT != getLeaderUnitType())
		{
			...
			{
				//if (GET_PLAYER((PlayerTypes)iI).isAlive())
				// srpt leaderhead project this won't matter
				if (GET_PLAYER((PlayerTypes)iI).isAlive() && GC.getGameINLINE().getGameTurn() > getTurnForYear(startingYear[iI])) // edead
				{
					//szBuffer = gDLL->getText("TXT_KEY_MISC_GENERAL_KILLED", getNameKey());
					szBuffer = gDLL->getText("TXT_KEY_MISC_GENERAL_KILLED_RFCCW", GET_PLAYER(getOwnerINLINE()).getCivilizationAdjectiveKey()); // srpt
	...
	{
		// srpt Slavery
		if (eCaptureUnitType == UNIT_SETTLER && GET_PLAYER(eCapturingPlayer).getCivics((CivicOptionTypes)2) == CIVIC_SLAVERY)
		{
		...
		if (eCaptureUnitType == UNIT_SLAVE && GET_PLAYER(eCapturingPlayer).getCivics((CivicOptionTypes)2) != CIVIC_SLAVERY)
		{
			eCaptureUnitType = ((UnitTypes)UNIT_WORKER);
		}
		// srpt end
...
void CvUnit::doTurn()
{
	...
	if (baseCombatStr() > 0)
	{
		...
		// srpt distant supply mod
		//if ((getDomainType() == DOMAIN_LAND) && (!isBarbarian()))
		//{
	...
	while (true)
	{
		...
		// edead: end part 2
		{
			if (getCombatFirstStrikes() == 0)
			{
				...
				{
					// srpt Flanking mod
					//if (m_pUnitInfo->getFlankingStrikeUnitClass(pDefender->getUnitClassType()) > 0)
					//{
		...
		if (isDead() || pDefender->isDead())
		{
			if (isDead())
			{
				...
				// edead: start global XP from Mongols
				bool bUpdateGlobal = true;
				//if (isBarbarian() && !isAlwaysFullXP()) bUpdateGlobal = false; // srpt barb xp counts fro GGs
			...
			else
			{
				...
				// edead: start global XP from Mongols
				bool bUpdateGlobal = true;
				//if (pDefender->isBarbarian() && !pDefender->isAlwaysFullXP()) bUpdateGlobal = false; // srpt barb xp counts for GGs
...
void CvUnit::updateCombat(bool bQuick)
{
	...
	if (bFinish)
	{
		...
		/*if (GC.getGameINLINE().getGameTurn() < getTurnForYear(startingYear[GC.getGameINLINE().getActivePlayer()]))*/ //srpt
		{
		...
		if (isDead())
		{
			...
			//CvEventReporter::getInstance().combatResult(pDefender, this); // srpt comment
			CvEventReporter::getInstance().combatResult(pDefender, this, this); // srpt slavery
		}
		else if (pDefender->isDead())
		{
			...
			// report event to Python, along with some other key state
			//CvEventReporter::getInstance().combatResult(this, pDefender); // srpt comment
			CvEventReporter::getInstance().combatResult(this, pDefender, this); // srpt slavery
...
bool CvUnit::canEnterTerritory(TeamTypes eTeam, bool bIgnoreRightOfPassage) const
{
	...
	/*if (getOwnerINLINE() == SATAVAHANA && AI_getUnitAIType() == UNITAI_MISSIONARY)
	{
		return true;
	}*/
	// end
	// srpt
	if ((baseCombatStr() == 0) && (workRate(false) == 0))
	{
		if ((eTeam == INDEPENDENT) || (eTeam == INDEPENDENT2) || (eTeam == INDEPENDENT3))
		{
			return true;
		}
	}
...
bool CvUnit::canMoveInto(const CvPlot* pPlot, bool bAttack, bool bDeclareWar, bool bIgnoreLoad) const
{
	...
	{
		// srpt missionaries, pilgrims and spies can enter snow
		if ((baseCombatStr() == 0) && (workRate(false) == 0) && (pPlot->getFeatureType() != 7) && (!pPlot->isPeak()) && (!pPlot->isWater()))
		{
			bMove = true;
		}
		if (bMove == false)
...
/************************************************************************************************/
	...
	/*case DOMAIN_LAND:
		if (pPlot->isWater() && !canMoveAllTerrain())
		{
			if (!pPlot->movementCost(this, plot()) == 0)
			{
...
bool CvUnit::canAirliftAt(const CvPlot* pPlot, int iX, int iY) const
{
	...
	if (!pTargetPlot->isCoastalLand()) // srpt naval transport idea
	{
		return false;
	}

	if (plotDistance(pPlot->getX_INLINE(), pPlot->getY_INLINE(), iX, iY) > 15)
	{
		return false;
	}
	
	if (artStyleMap[EARTH_Y -1 -pPlot->getY_INLINE()][pPlot->getX_INLINE()] != artStyleMap[EARTH_Y -1 -iY][iX])
	{
		if (artStyleMap[EARTH_Y -1 -pPlot->getY_INLINE()][pPlot->getX_INLINE()] == 4)
		{
			if (artStyleMap[EARTH_Y -1 -iY][iX] != 10)
			{
				if (artStyleMap[EARTH_Y -1 -iY][iX] != 11)
				{
					return false;
				}
			}
		}
		if (artStyleMap[EARTH_Y -1 -pPlot->getY_INLINE()][pPlot->getX_INLINE()] == 10)
		{
			if (artStyleMap[EARTH_Y -1 -iY][iX] != 5)
			{
				if (artStyleMap[EARTH_Y -1 -iY][iX] != 5)
				{
					return false;
		...
		if (artStyleMap[EARTH_Y -1 -pPlot->getY_INLINE()][pPlot->getX_INLINE()] == 11)
		{
			if (artStyleMap[EARTH_Y -1 -iY][iX] != 4)
			{
				if (artStyleMap[EARTH_Y -1 -iY][iX] != 6)
				{
					return false;
				}
			}
		}
		if ((artStyleMap[EARTH_Y -1 -pPlot->getY_INLINE()][pPlot->getX_INLINE()] == 5) && (artStyleMap[EARTH_Y -1 -iY][iX] != 10))
		{
			return false;
		}
	}
	...
	if (pPlot->getImprovementType() != NO_IMPROVEMENT)
	{
		...
		if (pPlot->getTeam() != getTeam())
		{
			...
			if (iPillageGold > 0)
			{
				...
				if (GC.getIDW_ENABLED() && GC.getIDW_PILLAGE_INFLUENCE_ENABLED() && (GC.getGameINLINE().getGameTurn() >= getTurnForYear(startingYear[GC.getGameINLINE().getActivePlayer()])))
				{
...
// edead: start quick pillage, for use with barbarian auto-pillage
...
bool CvUnit::canFound(const CvPlot* pPlot, bool bTestVisible) const
{
	...
	// edead: bugfix
	if (!GET_PLAYER(getOwnerINLINE()).isHuman() && pPlot->getMapFoundValue(getOwnerINLINE()) <= 3)
	{
...
int CvUnit::getMaxHurryProduction(CvCity* pCity) const
{
	...
	//if (getOwnerINLINE() == QIN && AI_getUnitAIType() == UNITAI_WORKER)
	//if (GET_PLAYER(getOwnerINLINE()).getLeaderType() == QIN_SHI_HUANG && AI_getUnitAIType() == UNITAI_WORKER)
	if (GET_PLAYER(getOwnerINLINE()).getLeaderType() == QIN_SHI_HUANG && getUnitClassType() == GC.getInfoTypeForString("UNITCLASS_SLAVE"))
	{
		iProduction *= 2;
	}
	// end
	// srpt: Archimedes effect: great priests can create great works
	if (GET_PLAYER(getOwnerINLINE()).isHasBuilding((BuildingTypes)ARCHIMEDES) && getUnitClassType() == GC.getInfoTypeForString("UNITCLASS_SCIENTIST"))
	{
...
bool CvUnit::canGoldenAge(const CvPlot* pPlot, bool bTestVisible) const
{
	...
	if (!bTestVisible)
	{ // srpt Guptan UP
		if (getOwnerINLINE() != GUPTA && (GET_PLAYER(getOwnerINLINE()).unitsRequiredForGoldenAge() > GET_PLAYER(getOwnerINLINE()).unitsGoldenAgeReady()))
		{
			return false;
		}
		if (getOwnerINLINE() == GUPTA && (GET_PLAYER(getOwnerINLINE()).unitsRequiredForGoldenAge() -1 > GET_PLAYER(getOwnerINLINE()).unitsGoldenAgeReady()))
		{
			return false;
		}
	} // srpt end
...
bool CvUnit::canUpgrade(UnitTypes eUnit, bool bTestVisible) const
{
	...
	if (isHasPromotion((PromotionTypes)PROMOTION_MERCENARY)) //Mercenary
	{
		return false;
	}
	//Rhye - end
	...
	if (pAttacker != NULL)
	{
		...
		if (isBarbarian())
		{
			if (pAttacker->isHuman())
			{
				...
				if (pAttacker->isHasPromotion((PromotionTypes)PROMOTION_DISCIPLINE)) iExtraModifier -= 20; // srpt
				iModifier += iExtraModifier;
				if (pCombatDetails != NULL)
				{
		...
		if (pAttacker->isBarbarian())
		{
			...
			{
				iExtraModifier = -GC.getHandicapInfo(GC.getGameINLINE().getHandicapType()).getBarbarianCombatModifier();
				if (isHasPromotion((PromotionTypes)PROMOTION_DISCIPLINE)) iExtraModifier += 20; // srpt
				iModifier += iExtraModifier;
				if (pCombatDetails != NULL)
				{
	...
	for (int i = -iBlockadeRange; i <= iBlockadeRange; ++i)
	{
		for (int j = -iBlockadeRange; j <= iBlockadeRange; ++j)
		{
			...
			if (NULL != pLoopPlot && pLoopPlot->isRevealed(getTeam(), false))
			{
				...
				if (NULL != pCity && !pCity->isPlundered())// && isEnemy(pCity->getTeam()) && !atWar(pCity->getTeam(), getTeam())) // srpt: blockading now plunders gold during wartime and from any unit
				{
					if (pCity->area() == area() || pCity->plot()->isAdjacentToArea(area()))
					{
						int iGold = pCity->calculateTradeProfit(pCity) * pCity->getTradeRoutes();
						if (getOwnerINLINE() == VANDALS) iGold *= 2; //srpt Vandal UP
						if (iGold > 0)
						{
...
void CvUnit::read(FDataStreamBase* pStream)
{
	...
	pStream->ReadString(m_szArtDefineTag); // edead // srpt indy unit art
...
void CvUnit::write(FDataStreamBase* pStream)
{
	...
	pStream->WriteString(m_szArtDefineTag); // edead // srpt indy unit art
