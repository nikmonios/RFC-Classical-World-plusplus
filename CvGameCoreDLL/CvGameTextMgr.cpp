...
//			}
	...
	else if (bShift && !bAlt && (gDLL->getChtLvl() > 0))
	{
		...
		szTempBuffer.Format(L"\nFound Value: %d, (Calc:%d, Start:%d, Map:%d)", iActualFoundValue, iCalcFoundValue, iStartingFoundValue, pPlot->getMapFoundValue(eActivePlayer)); // edead: debug
		szString.append(szTempBuffer);
			...
			if (kPlayer.isAlive())
            {
					...
					|| ((pPlot->getOwner() == iI) && (iBestAreaFoundValue > 0)))
				{
					...
					if (iCalcFoundValue > 0 || iStartingFoundValue > 0)
					{
						...
						szTempBuffer.Format(L" (Calc:%d,Start:%d,Map:%d)", iCalcFoundValue, iStartingFoundValue, pPlot->getMapFoundValue(ePlayer)); // edead: debug
						szString.append(szTempBuffer);
					}
...
*/		}
		...
		szInfoText.append(szTempString);*/ // srpt comment
	}
	else
	{
...
*/
		...
		szHelpText.append(gDLL->getText("TXT_KEY_CIVIC_MILITARY_HAPPINESS"));
/************************************************************************************************/
...
void CvGameTextMgr::setUnitHelp(CvWStringBuffer &szBuffer, UnitTypes eUnit, bool bCivilopediaText, bool bStrategyText, bool bTechChooserText, CvCity* pCity)
{
	...
	if ((pCity == NULL) || !(pCity->canTrain(eUnit)))
	{
		...
		if (!bCivilopediaText)
		{
			...
			if (bHasOrTechPrereq = true)
			{
				for (iI = 0; iI < GC.getNUM_UNIT_AND_TECH_PREREQS(); ++iI)
				{
					if (GC.getUnitInfo(eUnit).getPrereqAndTechs(iI) != NO_TECH)
					{
						if (bTechChooserText || GC.getGameINLINE().getActivePlayer() == NO_PLAYER || bHasOrTech == false)
						{
							szTempBuffer.Format(L"%s%s", NEWLINE, gDLL->getText("TXT_KEY_REQUIRES").c_str());
...
void CvGameTextMgr::setProcessHelp(CvWStringBuffer &szBuffer, ProcessTypes eProcess)
{
	...
	for (iI = 0; iI < NUM_COMMERCE_TYPES; ++iI)
	{
		if (GC.getProcessInfo(eProcess).getProductionToCommerceModifier(iI) != 0)
		{
			...
			//if (GC.getGame().getActivePlayer() == SATAVAHANA)
...
void CvGameTextMgr::parseGreatPeopleHelp(CvWStringBuffer &szBuffer, CvCity& city)
{
	...
	// edead: start srpt Egypt UP
	if (city.getOwner() == EGYPT && GET_PLAYER(city.getOwner()).getCurrentEra() < 2)
	{
		szBuffer.append(gDLL->getText("TXT_KEY_MISC_HELP_EGYPT_UP", 50));
		szBuffer.append(NEWLINE);
		iModifier += 50;
	}
	// edead: end
