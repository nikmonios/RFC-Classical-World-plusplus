...
// srpt start Techs allow specialists
bool CvTechInfo::isSpecialistValid(int i) const							
{
...
// edead: start
,m_bPurgeReligion(false)
,m_bAlwaysFullXP(false)
,m_iAutoPillageProb(0)
// edead: end
{
}
...
// edead: start
int CvUnitInfo::getOrStateReligion() const
{
...
// edead: start
int CvUnitInfo::getOrPrereqReligion() const			
{
...
// edead: start
bool CvUnitInfo::isPurgeReligion() const
{
	return m_bPurgeReligion;
}

bool CvUnitInfo::isAlwaysFullXP() const
{
	return m_bAlwaysFullXP;
}

int CvUnitInfo::getAutoPillageProb() const
{
	return m_iAutoPillageProb;
}
// edead: end
...
// edead: start
const TCHAR* CvUnitInfo::getArtDefineTag(int i, int eStyle) const
{
	return getEarlyArtDefineTag(i, (UnitArtStyleTypes)eStyle);
}
...
void CvUnitInfo::read(FDataStreamBase* stream)
{
	...
	stream->Read(&m_bPurgeReligion); // edead
	stream->Read(&m_bAlwaysFullXP); // edead
	stream->Read(&m_iAutoPillageProb); // edead
...
void CvUnitInfo::write(FDataStreamBase* stream)
{
	...
	stream->Write(m_bPurgeReligion); // edead
	stream->Write(m_bAlwaysFullXP); // edead
	stream->Write(m_iAutoPillageProb); // edead
...
bool CvUnitInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// edead: start
	pXML->GetChildXmlValByName(&m_bPurgeReligion,"bPurgeReligion",false);
	pXML->GetChildXmlValByName(&m_bAlwaysFullXP,"bAlwaysFullXP",false);
	pXML->GetChildXmlValByName(&m_iAutoPillageProb, "iAutoPillageProb",0);
	// edead: end
...
CvCivicInfo::~CvCivicInfo()
{
	...
	// srpt start Specialist bonus from Civics
bool CvCivicInfo::isAnySpecialistYieldChange() const
{
...
const int CvCivilizationInfo::getStartingYear() const // edead - changed to int
...
void CvCivilizationInfo::write(FDataStreamBase* stream)
{
	...
	// 3MiroCiv: New Atts
	stream->Write(m_iStartingYear);
	stream->Write(m_iStartingCalendarYear);//srpt
...
bool CvTerrainInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(&m_bWater, "bWater");
	pXML->GetChildXmlValByName(&m_bImpassable, "bImpassable");
	pXML->GetChildXmlValByName(&m_bSaline, "bSaline"); // edead
	pXML->GetChildXmlValByName(&m_bFound, "bFound");
...
m_iDiplomacyModifier(0), // edead - bonus to Diplomacy
...
// edead - start (returns bonus to Diplomacy)
int CvTraitInfo::getDiplomacyModifier() const									
{
...
bool CvTraitInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(&m_iDiplomacyModifier, "iDiplomacyModifier"); // edead - bonus to Diplomacy
