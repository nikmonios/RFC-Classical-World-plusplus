...
bool isPromotionValid(PromotionTypes ePromotion, UnitTypes eUnit, bool bLeader)
{
	...
	if (kUnit.isOnlyDefensive() && !kUnit.isSpy())//SuperSpies: TSHEEP End
	{
	...
	if (kUnit.getInterceptionProbability() == 0 && !kUnit.isSpy())//SuperSpies: TSHEEP End
	{
...
const wchar* wcrtrim(const wchar* s)
{
	if (s)
	{
		wchar* buf = const_cast<wchar*>(s);
		//const wchar* result = L"";
		int i = wcslen(buf);
		while ((--i)>0 && iswspace(buf[i])) buf[i] = 0;
		//wcscpy(buf, result);
		//const wchar* result = buf;
		//delete [] buf;
		//return result;
		return buf;
	}
