...
bool CvDLLButtonPopup::launchRazeCityPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	...
	// edead: start massacre of population part I srpt no massacre without state religion
	bool bMassacre = false;
	if ((player.getStateReligion() != NO_RELIGION) && (player.getStateReligion() != JAINISM))
	{
		if (pNewCity->getReligionCount() > 0)
		{
			if ((!pNewCity->isHasReligion(player.getStateReligion())) || (pNewCity->getReligionCount() > 1))
			{
				bMassacre = true;
			}
		}
	...
	if (bRaze)
	{
		if (player.getCivics((CivicOptionTypes)2) == CIVIC_SLAVERY)
		{
			gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, gDLL->getText("TXT_KEY_POPUP_ENSLAVE_CAPTURED_CITY").c_str(), NULL, 1, WIDGET_GENERAL);
		}
		else
		{
