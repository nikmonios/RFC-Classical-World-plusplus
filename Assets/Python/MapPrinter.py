...
tWBFilename = "C:\Users\samuelrpetite\Documents\My Games\Beyond the Sword\Saves\WorldBuilder\settler maps\print map\map.CivBeyondSwordWBSave"
...
def populate_array_settler(rows, columns):
#	array_dic = {}
#	for row in range(1, rows+1):		## starts with one, not zero
#		array_dic[row] = []
#		for col in range(0, columns):
#			array_dic[row].append('None')		## initialize to 'None'
	return [[3 for i in range(columns)] for j in range(rows)]
...
def write_array_to_PythonSettler(rows, columns, array ):
	for iY in range( iMapMaxY ):
		row = ''
		row += "{"
		for iX in range( iMapMaxX ):
...
def printSettlerMap():
	tProv = populate_array_settler( iMapMaxY, iMapMaxX )
	f = open(tWBFilename, "r")
	bSignStart = False
	bWaitTerrain = False
	bMountNotSet = False
	...
	for line in f:
		...
		if ( "caption=" in line ):
			...
			#tProv[iY][iX] = line[9:]
			tProv[iY][iX] = 300
	write_array_to_PythonSettler( iMapMaxY, iMapMaxX, tProv )
