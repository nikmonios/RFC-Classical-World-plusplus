...
from string import rstrip #edead
...
# their promotions.
# Default value is 10
g_iMercenaryPromotionChance = 4 #srpt
...
g_dMaintenanceCostModifier = 0.25 #srpt

g_dBaseMaintenanceCost = 2 #srpt
...
# Default value is 1
g_iMinimumStartingMercenaryPromotionCount = 1 #srpt
	...
	def getEraAppropriateRandomMercenary(self, iEra):
		...
		while(len(promotionList) < g_iMinimumStartingMercenaryPromotionCount):
			...
			while(bLoop): #Rhye
				...
				#if iUnitType == con.iPaulicianAxeman:
					#tempDict = sd.getMercenaryData("MercenaryNames")
					#if tempDict.has_key("Paulician Axeman"):
						#continue
				#elif iUnitType == con.iKhwarezmianLancer:
					#if gc.getPlayer(con.iKushans).isAlive() or gc.getGame().getGameTurnYear() > 1260:
						#continue
					#tempDict = sd.getMercenaryData("MercenaryNames")
					#if tempDict.has_key("Khwarezmian Lancer"):
						#continue
				...
				for i in range(len(unitTechPrereqList)):
					...
					if ((iCounter+1)*2 < gc.getGame().countCivPlayersAlive()):
						...
						bContinue = True
						break
				if (self.getMaxTechEra(unitTechPrereqList) < iEra-1): #accept current and previous era only, no more warriors in later ages
...
##			        #Rhye - end
	...
	def getSpecificMercenary(self, iPlayer, iUnitType, strName, iX, iY):
		...
		hiringPlayer = gc.getPlayer(gc.getGame().getActivePlayer())
		...
		if(iUnitType != -1):
			# Create the mercenary unit
			tmpUnit = hiringPlayer.initUnit(iUnitType, con.iMercX, con.iMercY, UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH) # edead
	...
	def getVirtualMercenaryCost(self, iUnitType, strName):
		' Mercenary - the mercenary'
		barbarianPlayer = gc.getPlayer(gc.getGame().getActivePlayer())
	...
	def setInitialMercenaryExperience(self, objMercenary, promotionList):
		...
		# Return immediately if the promotionList is not set
		...
		objMercenary.setLevel(iPromotionCount) # edead - xp bugfix
		
		# Get the experience level of the mercenary
		# edead - start bugfix for wrong XP
		#iMercenaryExperience = objMercenary.experienceNeeded()
		if (iPromotionCount == 0):
			iMercenaryExperience = 0
		else:
			iMercenaryExperience = objMercenary.experienceNeeded()
		# edead - end
		
		# Reset the mercenary level to their real level
		objMercenary.setLevel(iPromotionCount+1) # edead - xp bugfix
		...
		objMercenary.setExperience(iMercenaryExperience+iRandomBonusExperience, objMercenary.experienceNeeded()) # edead - xp bugfix
	...
	def getUnitPromotionList(self, objUnit):
		...
		for i in range(gc.getNumPromotionInfos()):
			...
			# srpt: no more super units
			if i in [iDiscipline, iSilkworm, iCombat3, iCombat4, iCombat5, iAmphibious, iMedic1, iMedic2, iGuerilla3, iWoodsman3, iCityRaider3, iCityGarrison3, iDrill3, iDrill4, iBarrage3, iAccuracy, iFlanking3, iLeader, iLeadership, iTactics, iCombat6, iMorale, iMedic3, iFeintAttack, iEncirclement, iGreekFire, iDesertAdaptation, iBlessed, iRaider, iOceanfaring, iMercenary]:
				continue
			...
			#if (objUnit.isPromotionValid(iPromotionInfoType)):
			#if (isPromotionValid(iPromotionInfoType, objUnit.getUnitType(), False)): # Rhye ?
			if (isPromotionValid(i, objUnit.getUnitType(), False)): # edead
				if i not in [iDiscipline, iSilkworm, iCombat3, iCombat4, iCombat5, iAmphibious, iMedic1, iMedic2, iGuerilla3, iWoodsman3, iCityRaider3, iCityGarrison3, iDrill3, iDrill4, iBarrage3, iAccuracy, iFlanking3, iLeader, iLeadership, iTactics, iCombat6, iMorale, iMedic3, iFeintAttack, iEncirclement, iGreekFire, iDesertAdaptation, iBlessed, iRaider, iOceanfaring, iMercenary]:
	...
	def getEraAppropriateUnitPromotionList(self, objUnit, iEra):
		...
		for i in range(gc.getNumPromotionInfos()):
			...
			# edead: skip invalid promotions
			if not isPromotionValid(i, objUnit.getUnitType(), False):
				continue
				
			# srpt: no more super units
			if i in [con.iDiscipline, con.iSilkworm, con.iCombat3, con.iCombat4, con.iCombat5, con.iAmphibious, con.iMedic1, con.iMedic2, con.iGuerilla3, con.iWoodsman3, con.iCityRaider3, con.iCityGarrison3, con.iDrill3, con.iDrill4, con.iBarrage3, con.iAccuracy, con.iFlanking3, con.iLeader, con.iLeadership, con.iTactics, con.iCombat6, con.iMorale, con.iMedic3, con.iFeintAttack, con.iEncirclement, con.iGreekFire, con.iDesertAdaptation, con.iBlessed, con.iRaider, con.iOceanfaring, con.iMercenary]:
				continue
			...
			# edead - SoI start
			# 5-10 - Anti-*, 11 - Amphibious, 12 - March, 13 - Blitz, 14 - Commando, 15-16 - Medic
			# 17-19 - Guerilla, 20-22 - Woodsman, 23-25 - City Raider, 26-28 - City Garrison, 29-32 - Drill
			# 33-35 - Barrage, 36 - Accuracy, 37-39 - Flanking, 40 - Sentry, 41 - Mobility, 42-43 - Navigation
			# 44-49 - Great Warlord stuff, 50 - Feint, 51 - Encirclement, 52 - Greek Fire, 53 - Desert Adaptation
			# 54 - Blessed,  55 - Raider, 56 - Ocean-faring, 57 - Mercenary
			lPromotionOdds = [100, 80, 60, 40, 20, 60, 60, 60, 60, 60, 60, 20, 20, 20, 0, 40, 20, 100, 80, 60, 100, 80, 60, 100, 80, 60, 100, 80, 60, 100, 80, 60, 40, 100, 80, 60, 60, 100, 80, 60, 20, 60, 80, 60, 0, 0, 0, 0, 0, 0, 20, 0, 0, 0, 0, 0, 0, 0, 0]

			iRndNum = gc.getGame().getSorenRandNum(100, 'random')
			#print (gc.getNumPromotionInfos(), len(lPromotionOdds))
			# srpt this for now
			if iRndNum > 40:
			#if i < len(lPromotionOdds) and iRndNum > lPromotionOdds[i]: # edead
				continue
			#Rhye - end

			#Rhye - start (no anti-... before correct era)
			# edead - SoI values
			if ((i == 8 or i == 13 or i == 14 or i == 50) and gc.getActivePlayer().getCurrentEra() < 1): # Formation, Feint, Blitz, Commando
				continue
			if ((i == 15 or i == 16) and gc.getActivePlayer().getCurrentEra() < 2): # Medic I, Medic II
				continue
			if ((i == 7) and gc.getActivePlayer().getCurrentEra() < 3): # Pinch
				continue
			#Rhye - end	
			
			# edead - start per-unit type promotions
			#if objUnit.getUnitType() in [con.iZanjiSwordsman, con.iZanjiSpearman, con.iNaffatun, con.iFidai, con.iVarangianGuard, con.iPaulicianAxeman, con.iSpearman_African, con.iArcher_African]:
				#if i >= 20 and i <= 22: # woodsman
					#continue
			#if objUnit.getUnitType() in [con.iNaffatun, con.iFidai, con.iPashtunWarrior, con.iPashtunCavalry, con.iAfghanInfantry, con.iAfghanPikeman]:
				#if i == 11: # amphibious
					#continue
			# edead - end
			...
			# of promotionList
			#if(isPromotionValid(iPromotionInfoType, objUnit.getUnitType(), False) and bEraMatch): # Rhye ?
			
			promotionList.append(promotionInfo)
			
			# edead - start
			# Add Self-Preservation
	...
	def getMercenaryPromotions(self, rawPromotionList):
		...
		iMercenaryPromotionChance = g_iMercenaryPromotionChance # srpt + gc.getActivePlayer().getCurrentEra() #Rhye (stronger mercenaries later in the game)
	...
	def getAvailableMercenaries(self):
		...
		mercenaries = sd.getMercenaryData(AVAILABLE_MERCENARIES) # edead
	...
	def getPlayerMercenaryMaintenanceCost(self, iPlayer):
		...
		mercenaries = sd.getMercenaryData(HIRED_MERCENARIES) # edead
		...
		if (not gc.getPlayer(iPlayer).isHuman()):
			totalMercenaryMaintenanceCost *= 100 - (gc.getGame().getHandicapType())*25
	...
	def getUnplacedContractedOutUnits(self, iPlayer):
		...
		mercenaries = sd.getMercenaryData(UNPLACED_MERCENARIES) # edead 
	...
	def hireMercenary(self, mercenaryName, iPlayer):
		...
		sd.setMercenaryData(AVAILABLE_MERCENARIES, mercenaries) # edead
		...
		#if (iPlayer != gc.getGame().getActivePlayer()): # edead
	...
	def placeMercenaries(self, iPlayer):
		...
		for mercenary in mercenariesToPlace:
			...
			if mercenaries.has_key(tmpMercenary.getName()): # edead: bugfix
				del mercenaries[tmpMercenary.getName()]
			
			# Get the starting location for the mercenary
			if tmpMercenary.getUnitInfo().getDomainType() == gc.getInfoTypeForString("DOMAIN_LAND"):
				objCity = self.getMercenaryStartingLocation(iPlayer)
			else:
				objCity = self.getCoastalStartingLocation(iPlayer)
		...
		sd.setMercenaryData(UNPLACED_MERCENARIES, mercenaries) # edead
	...
	def addMercenariesToPool(self):
		...
		mercenaryNames  = sd.getMercenaryData(MERCENARY_NAMES) # edead
		...
		maxMercenaryCount = gc.getGame().getMapRand().get(self.getMaxMercenaryCreationCount(), "Max Mercenary Count") + self.getMinMercenaryCreationCount()
		...
		sd.setMercenaryData(MERCENARY_NAMES, mercenaryNames) # edead
		#print("Mercenaries Mod", "MercenaryData", MERCENARY_NAMES, mercenaryNames)
		
		
	# Creates a specific mercenary at most being the value passed in the 
	# g_iMaxMercenaryCreationCount variable.
	def addSpecificMercenaryToPool(self, iUnitType, strName):
	...
	# edead: for mercenary galleys
	def getCoastalStartingLocation(self, iPlayer):
		"""Returns a coastal city that is the closest to the capital."""
		
		iBestDistance = 100
		pBestCity = None
		pCapital = gc.getPlayer(iPlayer).getCapitalCity()
		
		if pCapital.isCoastal(gc.getMIN_WATER_SIZE_FOR_OCEAN()):
			pBestCity = pCapital
		else:
			...
			for pCity in apCityList:
				...
				if city.isCoastal(gc.getMIN_WATER_SIZE_FOR_OCEAN()):
					iDistance = utils.calculateDistance(pCapital.getX(), pCapital.getY(), city.getX(), city.getY())
	...
	def getAtWarCivilizations(self, iPlayer):
		...
		for pyPlayer in playerList:
			...
			if(playerTeam.isAtWar(cyPlayer.getTeam())):
				# edead: start
				if cyPlayer.getID() < con.iNumPlayers and cyPlayer.getID() != iPlayer:
	...
	def getBestAvailableMercenary(self, iGold, iPlayer):
		...
		for mercenaryName in mercenaryDict:
			...
			# edead: start regional recruitment
	...
	def computerPlayerThink(self, iPlayer):
		...
		if(currentGold - mercenaryMaintenanceCost < 0):
			while(currentGold - mercenaryMaintenanceCost < 0):
				...
				if mercenaryDict.has_key(mercenary.getName()): # edead: bugfix
		...
		if player.getGold() > 800: iHireOdds -= 25 # edead
		elif player.getGold() > 400: iHireOdds -= 10 # edead
		...
		for i in range(randomMercCount):
			...
			# edead: start fix for Rhye's gold bug
			#currentGold = currentGold - mercenary.getHireCost()
			hireCost = mercenary.getHireCostXPlayer(iPlayer)
			hireCost *= 100 - (gc.getGame().getHandicapType())*25
			hireCost /= 100
			currentGold = currentGold - hireCost
			# edead: end
			...
			# Debug code - start
			if(g_bDebug):
				CvUtil.pyPrint(player.getName() + " hire cost: " + str(hireCost) + " final gold: " + str(player.getGold()))
			# Debug code - end
	...
	def getAvailableUnits(self, iPlayer):
		...
		for unit in unitList:
			...
			# If we can't contract out the unit then continue
			if(not self.canContractOutUnit(unitInfo, false)):
				continue
	...
	def canContractOutUnit(self, objUnitInfo, bNavalUnits = true):
		...
		if(objUnitInfo.getDomainType() != gc.getInfoTypeForString("DOMAIN_LAND")):
			if not (bNavalUnits and objUnitInfo.getType() in ["UNIT_WAR_GALLEY", "UNIT_GALLEY", "UNIT_ROUNDSHIP", "UNIT_DHOW", "UNIT_BAGHLAH"]): # edead: make an exception
				return false

## edead - start (redundant)
...
## edead - end
		...
		# if objUnitInfo.getID() not in con.lMercenaries: # edead
			# return false
	...
	def setupMercenaryData(self):
		...
		return # edead (moved to StoredData)
	...
	# edead: start
	def getMaxMercenaryCreationCount(self):
		i = g_iMaxMercenaryCreationCount
		if CyGame().getGameTurnYear() < 1000 or CyGame().getGameTurnYear() > 1300: 
			i -= 1
		if CyGame().getGameTurnYear() >= 1080 and CyGame().getGameTurnYear() <= 1220: 
			i += 1
		return i
	
	def getMinMercenaryCreationCount(self):
		i = g_iMinMercenaryCreationCount
		if CyGame().getGameTurnYear() < 1000 or CyGame().getGameTurnYear() > 1300: 
			i -= 1
		return i
	# edead: end
...
class Mercenary:
	...
	def __init__(self, mercenaryName, objUnitInfo, promotionList, iExperienceLevel, iNextExperienceLevel):
		...
		self.iLevel = len(promotionList) + 1 # edead - xp bugfix
	...
	def place(self, objCity):
		...
		# edead: return nothing if city is gone somehow
		if not objCity or objCity.isNone():
			return
		...
		if(g_bDisplayMercenaryMessages):
			strMessage = rstrip(self.getName()) + " has arrived at " + objCity.getName() + "."
			# Inform the player that the mercenary has arrived.
			CyInterface().addMessage(self.iOwner, False, con.iDuration, strMessage, "", 0, self.objUnitInfo.getButton(), ColorTypes(0), objCity.getX(), objCity.getY(), True, True) 
	...
	def fire(self):
		...
		if(g_bDisplayMercenaryMessages and self.iBuilder != -1 and self.iBuilder != self.iOwner):
			strMessage = rstrip(self.getName()) + " is no longer needed by " + gc.getPlayer(self.iOwner).getName()
	...
	def getHireCost(self):
		...
		if(self.objUnitInfo != None):
			self.iHireCost = (self.objUnitInfo.getProductionCost() * 2 + self.objUnitInfo.getProductionCost() * (self.getLevel()+1)) + ((self.getLevel()+1) * int(math.fabs(self.getExperienceLevel() - self.getNextExperienceLevel())))
			self.iHireCost /= 2
	...
	def getMercenaryMaintenanceCost(self):
		...
		return int((math.ceil(self.getLevel() * 1.5 + self.getHireCost() * 0.1))*dMaintenanceCostModifier) + g_dBaseMaintenanceCost #Rhye
	...
	def setExperience(self):
		...
		#self.objUnit.setExperience(self.iExperienceLevel, -1) # edead
	...
	# edead: start mod-specific conditions
	def canHireUnit(self, iPlayer, bNavalUnits = True):
		
		unitType = self.getUnitInfoID()
		iStateReligion = gc.getPlayer(iPlayer).getStateReligion()
		
		# make a list of owned regions; count coastal cities for naval units
		regionList = []
		bHasPorts = False
		apCityList = PyPlayer(iPlayer).getCityList()
		for pCity in apCityList:
			city = pCity.GetCy()
			regionList.append(city.plot().getRegionID())
			if bNavalUnits and not bHasPorts:
				if city.isCoastal(gc.getMIN_WATER_SIZE_FOR_OCEAN()):
					bHasPorts = True
		
		### BOATS ###
		if unitType == con.iGalley or unitType == con.iRoundship or unitType == con.iWarGalley:
			return bNavalUnits and bHasPorts and set(regionList) & con.mercRegions[con.iArea_Mediterranean]
		elif unitType == con.iDhow or unitType == con.iBaghlah:
			return bNavalUnits and bHasPorts and set(regionList) & con.mercRegions[con.iArea_IndianOcean]
		
		### JAVELINMEN ###
		elif unitType == con.iJavelinman_Mediterranean:
			return set(regionList) & con.mercRegions[con.iArea_Mediterranean]
			
		elif unitType == con.iJavelinman_Persian:
			return set(regionList) & con.mercRegions[con.iArea_Persian]
			
		elif unitType == con.iJavelinman_East_Asian:
			return set(regionList) & con.mercRegions[con.iArea_East_Asian]
			
		elif unitType == con.iJavelinman_Indian:
			return set(regionList) & con.mercRegions[con.iArea_Indian]
			
		elif unitType == con.iJavelinman_African:
			return set(regionList) & con.mercRegions[con.iArea_African]
			
		elif unitType == con.iSlinger:
			return set(regionList) & con.mercRegions[con.iSlinger]
			
		### AXEMEN ###
		
		elif unitType == con.iAxeman:
			return set(regionList) & con.mercRegions[con.iAxeman]
			
		elif unitType == con.iAxeman_Teutonic:
			return set(regionList) & con.mercRegions[con.iAxeman_Teutonic]
			
		elif unitType == con.iAxeman_Indian:
			return set(regionList) & con.mercRegions[con.iAxeman_Indian]
			
		elif unitType == con.iAxeman_East_Asian:
			return set(regionList) & con.mercRegions[con.iAxeman_East_Asian]
		
		### SPEARMEN ###
		elif unitType == con.iSpearman:
			return set(regionList) & con.mercRegions[con.iSpearman]
			
		elif unitType == con.iSpearman_East_Asian:
			return set(regionList) & con.mercRegions[con.iSpearman_East_Asian]
			
		elif unitType == con.iSpearman_African:
			return set(regionList) & con.mercRegions[con.iSpearman_African]
			
		elif unitType == con.iSpearman_European:
			return set(regionList) & con.mercRegions[con.iSpearman_European]
			
		elif unitType == con.iSpearman_Indian:
			return set(regionList) & con.mercRegions[con.iSpearman_Indian]
		
		### HEAVY SPEARMEN ###
		elif unitType == con.iHeavySpearman:
			return set(regionList) & con.mercRegions[con.iHeavySpearman]
			
		elif unitType == con.iHeavySpearman_Greek:
			return set(regionList) & con.mercRegions[con.iHeavySpearman_Greek]
			
		elif unitType == con.iHeavySpearman_Athenian:
			return set(regionList) & con.mercRegions[con.iHeavySpearman_Athenian]
			
		elif unitType == con.iHeavySpearman_Corinthian:
			return set(regionList) & con.mercRegions[con.iHeavySpearman_Corinthian]
			
		elif unitType == con.iHeavySpearman_Spartan:
			return set(regionList) & con.mercRegions[con.iHeavySpearman_Spartan]
			
		#elif unitType == con.iHeavySpearman_European:
			#return set(regionList) & con.mercRegions[con.iHeavySpearman_European]
			
		elif unitType == con.iHeavySpearman_Arab:
			return set(regionList) & con.mercRegions[con.iHeavySpearman_Arab]
			
		elif unitType == con.iHeavySpearman_Indian:
			return set(regionList) & con.mercRegions[con.iHeavySpearman_Indian]
			
		elif unitType == con.iHeavySpearman_East_Asian:
			return set(regionList) & con.mercRegions[con.iHeavySpearman_East_Asian]
		
		### ARCHERS ###
		elif unitType == con.iArcher:
			return set(regionList) & con.mercRegions[con.iArcher]
			
		elif unitType == con.iArcher_East_Asian:
			return set(regionList) & con.mercRegions[con.iArcher_East_Asian]
			
		elif unitType == con.iArcher_Indian:
			return set(regionList) & con.mercRegions[con.iArcher_Indian]
			
		elif unitType == con.iArcher_African:
			return set(regionList) & con.mercRegions[con.iArcher_African]
			
		elif unitType == con.iArcher_European:
			return set(regionList) & con.mercRegions[con.iArcher_European]
			
		elif unitType == con.iArcher_Greek:
			return set(regionList) & con.mercRegions[con.iArcher_Greek]
			
		elif unitType == con.iArcher_Arab:
			return set(regionList) & con.mercRegions[con.iArcher_Arab]
			
		### MARKSMEN ###
		elif unitType == con.iMarksman:
			return set(regionList) & con.mercRegions[con.iMarksman]	
			
		elif unitType == con.iMarksman_East_Asian:
			return set(regionList) & con.mercRegions[con.iMarksman_East_Asian]	
			
		elif unitType == con.iMarksman_Indian:
			return set(regionList) & con.mercRegions[con.iMarksman_Indian]	
			
		elif unitType == con.iMarksman_Persian:
			return set(regionList) & con.mercRegions[con.iMarksman_Persian]	
		
		### SWORDSMEN ###
		elif unitType == con.iSwordsman:
			return set(regionList) & con.mercRegions[con.iSwordsman]	
			
		elif unitType == con.iSwordsman_East_Asian:
			return set(regionList) & con.mercRegions[con.iSwordsman_East_Asian]
			
		elif unitType == con.iSwordsman_Indian:
			return set(regionList) & con.mercRegions[con.iSwordsman_Indian]
		
		### HEAVY INFANTRY ###
		elif unitType == con.iHeavySwordsman:
			return set(regionList) & con.mercRegions[con.iHeavySwordsman]
			
		elif unitType == con.iHeavySwordsman_East_Asian:
			return set(regionList) & con.mercRegions[con.iHeavySwordsman_East_Asian]
			
		elif unitType == con.iHeavySwordsman_Indian:
			return set(regionList) & con.mercRegions[con.iHeavySwordsman_Indian]
		
		### SKIRMISHERS ###
		elif unitType == con.iSkirmisher:
			return set(regionList) & con.mercRegions[con.iJavelinman]
			
		### HORSEMEN ###
		elif unitType == con.iHorseman:
			return set(regionList) & con.mercRegions[con.iHorseman]
		elif unitType == con.iHorseman_Scythian:
			return set(regionList) & con.mercRegions[con.iArea_Scythian]
		elif unitType == con.iHorseman_Sarmatian:
			return set(regionList) & con.mercRegions[con.iArea_Sarmatian]
		elif unitType == con.iHorseman_Xiongnu:
			return set(regionList) & con.mercRegions[con.iArea_Mongolian]
		elif unitType == con.iNumidianCavalry:
			return set(regionList) & con.mercRegions[con.iNumidianCavalry]
		
		### HORSE ARCHERS ###
		elif unitType == con.iHorseArcher:
			return set(regionList) & con.mercRegions[con.iHorseArcher]
		elif unitType == con.iHorseArcher_Scythian:
			return set(regionList) & con.mercRegions[con.iArea_Scythian]
		elif unitType == con.iHorseArcher_Sarmatian:
			return set(regionList) & con.mercRegions[con.iArea_Sarmatian]
		elif unitType == con.iHorseArcher_Xiongnu:
			return set(regionList) & con.mercRegions[con.iArea_Mongolian]
		
		### HEAVY HORSE ARCHERS ###
		elif unitType == con.iHeavyHorseArcher:
			return set(regionList) & con.mercRegions[con.iHorseArcher]
		
		### LANCERS ###
		elif unitType == con.iLancer:
			return set(regionList) & con.mercRegions[con.iHorseman]
		
		### HEAVY LANCERS ###
		elif unitType == con.iHeavyLancer:
			return set(regionList) & con.mercRegions[con.iHorseman]
