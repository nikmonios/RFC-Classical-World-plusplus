...
from Consts import tBirth, tVictory # edead: UHV score fix
...
def getScoreComponent(iRawScore, iInitial, iMax, iFactor, bExponential, bFinal, bVictory, iCiv=-1): # edead
	...
	if bFinal and bVictory:
		# edead: start
		iStartTurn = 0
		iEstimateEndTurn = gc.getGame().getEstimateEndTurn()
		if iCiv >= 0:
			if tBirth[iCiv] > 0:
				iStartTurn = getTurnForYear(tBirth[iCiv])
			iEstimateEndTurn = (gc.getGame().getEstimateEndTurn() + getTurnForYear(tVictory[iCiv])) / 2
		fTurnRatio = min(float(gc.getGame().getGameTurn() - iStartTurn) / float(iEstimateEndTurn - iStartTurn), 1.0)
		#fTurnRatio = float(gc.getGame().getGameTurn()) / float(gc.getGame().getEstimateEndTurn())
		#print "fTurnRatio:%.3f" %(fTurnRatio)
		# edead: end
