...
class CvPediaMain( CvPediaScreen.CvPediaScreen ):
	...
	def placeTechs(self):
		...
		for item in list:
			...
			###invisible dummy tech start part 2
                        if gc.getTechInfo(item[1]).getGridX()<=0 or gc.getTechInfo(item[1]).getGridY()<=0:                                
                                continue
                        ###invisible dummy tech end part 2 				
	...
	def placeCivs(self):
		...
		# srpt removal of non-playable civs
		listCopy = list[:]
		for item in listCopy:
		...
		for item in list:
			iRow = iCounter % nRows
			iColumn = iCounter // nRows
			if (gc.getCivilizationInfo(item[1]).isPlayable()): # srpt
			#if True: # edead
				if iRow >= iNumRows:
					iNumRows += 1
					screen.appendTableRow(tableName)
