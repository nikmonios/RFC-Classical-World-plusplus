...
#		ExoticForPrint ("# players = %d" % self.nCount)
		...
		nCount = 0
		offsetBeforeActiveLeader = 1
		for iLoopPlayer in range (gc.getMAX_PLAYERS()):
			if self.ltPlayerMet[iLoopPlayer]:
				if (iLoopPlayer != self.iActiveLeader):
					szName = self.getNextWidgetName()
					screen.addCheckBoxGFCAt(self.GLANCE_HEADER, szName, gc.getLeaderHeadInfo(gc.getPlayer(iLoopPlayer).getLeaderType()).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), self.X_GLANCE_OFFSET + (self.nSpread * (nCount+offsetBeforeActiveLeader)), self.Y_GLANCE_OFFSET, self.GLANCE_BUTTON_SIZE, self.GLANCE_BUTTON_SIZE, WidgetTypes.WIDGET_LEADERHEAD, iLoopPlayer, self.iActiveLeader, ButtonStyles.BUTTON_STYLE_LABEL, False)
				...
				if self.ltPlayerMet[j]:
					if j != self.iActiveLeader:
						...
						if nAttitude != None:
							...
							screen.setTextAt (szName, playerPanelName, szText, CvUtil.FONT_CENTER_JUSTIFY, self.X_GLANCE_OFFSET + (self.nSpread * (nCount+offsetBeforeActiveLeader)), self.Y_GLANCE_OFFSET + (self.GLANCE_BUTTON_SIZE / 2), -0.1, FontTypes.GAME_FONT, WidgetTypes.WIDGET_LEADERHEAD, j, iLoopPlayer)
						else:
							szText = localText.getText("TXT_KEY_FOREIGN_ADVISOR_NONE", ())
							screen.setTextAt (szName, playerPanelName, szText, CvUtil.FONT_CENTER_JUSTIFY, self.X_GLANCE_OFFSET + (self.nSpread * (nCount+offsetBeforeActiveLeader)), self.Y_GLANCE_OFFSET + (self.GLANCE_BUTTON_SIZE / 2), -0.1, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
					else:
						offsetBeforeActiveLeader = 0
					nCount += 1
