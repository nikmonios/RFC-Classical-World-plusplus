...
class CvMilitaryAdvisor:
	...
	# srpt
	def drawForceLimit(self):
		if gc.getPlayer(self.iActivePlayer).getTotalPopulation() <= 10:
			iArmyLimit = (gc.getPlayer(self.iActivePlayer).getTotalPopulation()) + 5
		else:
			iArmyLimit = (gc.getPlayer(self.iActivePlayer).getTotalPopulation()-10) / 2 + 15
		if gc.getTeam(gc.getPlayer(self.iActivePlayer).getTeam()).isHasTech(con.iLogistics):
			iArmyLimit  += iArmyLimit / 4
	
		iCoastalPopulation = 0
		apCityList = PyPlayer(self.iActivePlayer).getCityList()
		for pLoopCity in apCityList:
			if pLoopCity.GetCy().isCoastal(gc.getMIN_WATER_SIZE_FOR_OCEAN()):
				iCoastalPopulation += pLoopCity.getPopulation()
		if iCoastalPopulation <= 10:
			iNavyLimit = iCoastalPopulation + 3
		else:
			iNavyLimit = ((iCoastalPopulation - 10) / 2) + 13
		if gc.getTeam(gc.getPlayer(self.iActivePlayer).getTeam()).isHasTech(con.iLogistics):
			iNavyLimit += iNavyLimit / 4
				
		iMilitaryUnits = 0
		iNavyUnits = 0
		for iUnitClass in range(gc.getNumUnitClassInfos()):
			iDefaultUnit = gc.getUnitClassInfo(iUnitClass).getDefaultUnitIndex()
			if gc.getUnitInfo(iDefaultUnit).getDomainType() == gc.getInfoTypeForString("DOMAIN_LAND"):
				if gc.getUnitInfo(iDefaultUnit).isMilitaryProduction():
					iMilitaryUnits += gc.getPlayer(self.iActivePlayer).getUnitClassCountPlusMaking(iUnitClass)
			elif gc.getUnitInfo(iDefaultUnit).getDomainType() == gc.getInfoTypeForString("DOMAIN_SEA"):
				if gc.getUnitInfo(iDefaultUnit).isMilitaryProduction():
					iNavyUnits += gc.getPlayer(self.iActivePlayer).getUnitClassCountPlusMaking(iUnitClass)
				
			
		screen = self.getScreen()
		#screen.setLabel(self.FORCE_LIMIT_LABEL_ID, "", localText.getText("Force Limit:", ()), CvUtil.FONT_CENTER_JUSTIFY, self.X_GREAT_GENERAL_BAR + 300 + self.W_GREAT_GENERAL_BAR/2, self.Y_GREAT_GENERAL_BAR + 6, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_HELP_GREAT_GENERAL, -1, -1)
		screen.setLabel(self.FORCE_LIMIT_LABEL_ID, "", localText.getText("Army Force Limit:  " + str(iMilitaryUnits) + "/" + str(iArmyLimit) + "       " + "Navy Force Limit:  " + str(iNavyUnits) + "/" + str(iNavyLimit), ()), CvUtil.FONT_CENTER_JUSTIFY, self.X_GREAT_GENERAL_BAR + 400 + self.W_GREAT_GENERAL_BAR/2, self.Y_GREAT_GENERAL_BAR + 6, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_HELP_GREAT_GENERAL, -1, -1)
