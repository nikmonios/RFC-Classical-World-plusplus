...
from math import pow #Rhye/edead
...
##Rhye - begin	
		...
		screen.addStackedBarGFC("ProgressBar", 300, 415, 435, 30, InfoBarTypes.NUM_INFOBAR_TYPES, WidgetTypes.WIDGET_GENERAL, -1, -1)
...
##Rhye - begin
		if con.tBirth[CyGame().getActiveTeam()] <= gc.getGame().getGameTurnYear(): # edead
			...
			screen.setLabel("Text", "", CyTranslator().getText("TXT_KEY_AUTOPLAY_TURNS_REMAINING", (0,)), CvUtil.FONT_CENTER_JUSTIFY, 530, 450, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
			...
			screen = CyGInterfaceScreen( "CvLoadingScreen", self.iScreenID )
			exponent = 1 + iNumAutoPlayTurns/utils.getTurns(450)
			screen.setBarPercentage("ProgressBar", InfoBarTypes.INFOBAR_STORED, float(pow(iGameTurn, exponent)) / float(pow(iNumAutoPlayTurns, exponent)))
			screen.setLabel("Text", "", CyTranslator().getText("TXT_KEY_AUTOPLAY_TURNS_REMAINING", (max(0,iNumTurnsRemaining),)), CvUtil.FONT_CENTER_JUSTIFY, 530, 450, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
